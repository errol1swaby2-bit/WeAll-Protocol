...
def exact_bool_wrappers(path: Path, keys: set[str]) -> None:
    """Replace bool(mapping.get(<boolean-key>)) with exact-True checks.

    Accept arbitrary whitespace/newlines between bool( and the get() call. The
    earlier sweep only matched the single-line spelling and therefore missed
    multiline readiness fields in the controlled-testnet gate.
    """
    text = path.read_text(encoding="utf-8")
    before = text
    for key in sorted(keys):
        pattern = re.compile(
            rf'bool\(\s*([A-Za-z_][A-Za-z0-9_]*)\.get\("{re.escape(key)}"\)\s*\)',
            re.MULTILINE,
        )
...
# Sweep the rest of the blocker-report summaries too. This catches secondary
# machine-readable summaries such as external_operator_transcript_requirements
# instead of hardening only the top-level aggregate.
exact_bool_wrappers(blocker, boolean_keys)
