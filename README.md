...
Use `Weall-Protocol/configs/public_testnet_seed_registry.example.json`, `Weall-Protocol/scripts/sign_public_seed_registry_v1_5.py`, `Weall-Protocol/scripts/boot_public_observer_testnet.sh`, and `Weall-Protocol/docs/PUBLIC_OBSERVER_TESTNET_QUICKSTART.md` before claiming public observer launch readiness. The testnet remains resettable and non-economic; observer access can be open, but validator activation, signing authority, storage provider status, juror roles, governance authority, and reputation-sensitive roles remain protocol-gated.
