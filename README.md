...
| Governance/dispute quorum and reputation posture | `Weall-Protocol/src/weall/runtime/phase_progression.py`; `Weall-Protocol/src/weall/runtime/reputation_progression.py`; `Weall-Protocol/docs/GOVERNANCE_DISPUTE_PHASE_PROGRESSION.md`; `Weall-Protocol/docs/REPUTATION_PROGRESSION_AND_ANTI_FARMING.md` | phase-open quorum snapshot and anti-farming progression checks | tx/API contract maps and reputation action map | `PYTHONPATH=src python -m pytest -q tests/test_governance_dispute_phase_reputation_fairness.py`; `cd web && npm run test:phase-quorum-reputation-source` | Bounded public governance/dispute/reputation hardening | Not production constitutional governance or production-grade Sybil resistance readiness. |
