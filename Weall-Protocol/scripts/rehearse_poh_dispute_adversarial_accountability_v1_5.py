...
def _env(
    ...
    *,
    ...
    return TxEnvelope(
        ...
        signer=signer,
        nonce=nonce,
        chain_id="batch547-accountability",
        payload=payload,
        sig="sig",
        system=system,
        parent=parent,
...
def _run_poh_accountability() -> dict[str, Any]:
    ...
    apply_poh(
        ...
        _env(
            ...
            1,
            {
                "case_id": "case-547",
                "account_id": "@subject",
                "evidence_commitment": "sha256:" + "d" * 64,
            },
        ),
    )
    apply_poh(
    ...
    apply_poh(
        ...
        _env(
            ...
            "SYSTEM",
            1,
            {"case_id": "case-547", "jurors": ["@reviewer", "@r2", "@r3"]},
            system=True,
            parent="poh:case",
        ),
    )
    for i, reviewer in enumerate(["@reviewer", "@r2", "@r3"], start=1):
        ...
        apply_poh(
            ...
            _env(
                ...
                reviewer,
                i + 10,
                {"case_id": "case-547", "verdict": "approve"},
            ),
        )
    opened = apply_poh(
        state,
    ...
    upheld = apply_poh(
        ...
        _env(
            ...
            "SYSTEM",
            31,
            {"challenge_id": opened["challenge_id"], "resolution": "upheld", "case_id": "case-547"},
            system=True,
            parent="poh:challenge",
        ),
    )
    dismissed_open = apply_poh(
        state,
        _env(
            "POH_CHALLENGE_OPEN", "@r3", 32, {"account_id": "@subject", "reason": "low-confidence"}
        ),
    )
    dismissed = apply_poh(
        state,
        _env(
            ...
            "SYSTEM",
            33,
            {"challenge_id": dismissed_open["challenge_id"], "resolution": "dismissed"},
            system=True,
            parent="poh:challenge",
        ),
    )
    ...
    rec = (
        state.get("poh", {})
        .get("reviewer_accountability", {})
        .get("by_reviewer", {})
        .get("@reviewer", {})
    )
    return {
        "ok": bool(
            reviewer.get("poh_reviewer_eligible") is False
            and rec.get("eligible_for_poh_review") is False
            and dismissed.get("consequence", {}).get("type") == "none"
        ),
        ...
        "reverification_required": state.get("poh", {})
        .get("reverification", {})
        .get("by_account", {})
        .get("@subject", {})
        .get("status")
...
def _run_dispute_accountability_and_remedy() -> dict[str, Any]:
    ...
    first = apply_dispute(
        state,
        _env(
            "DISPUTE_FINAL_RECEIPT", "SYSTEM", 4, {"dispute_id": "d-547"}, system=True, parent="d"
        ),
    )
        ...
        _env(
            ...
            5,
            {
                ...
                "appeal_resolution": {
                    ...
                    "summary": "remedy",
                    "actions": [
                        {"tx_type": "ACCOUNT_REINSTATE", "payload": {"account_id": "@open"}},
                        {
    ...
    return {
        "ok": bool(
            juror_ineligible
            and target.get("restricted") is False
            and target.get("locked") is False
            and juror.get("dispute_juror_eligible") is True
        ),
        ...
        "juror_accountability_record": state.get("dispute_juror_accountability", {})
        .get("by_juror", {})
        .get("@j2", {}),
...
def run_harness() -> dict[str, Any]:
    ...
    return {
        ...
        "false_positive_remedy_path_present": bool(
            dispute.get("target_reinstated") and dispute.get("juror_eligible_after_remedy")
        ),
