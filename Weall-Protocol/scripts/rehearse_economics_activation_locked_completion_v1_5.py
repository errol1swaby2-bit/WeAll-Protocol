...
def _complete_locked_state() -> dict[str, Any]:
    return {
        ...
        "params": {
            ...
            "economics_strict_activation_preconditions_v2": True,
        },
...
def run_harness() -> dict[str, Any]:
    ...
    try:
        apply_economics(
            ...
            _env(
                ...
                "@alice",
                1,
                {"to_account_id": "@bob", "amount": 1, "transfer_id": "t-1"},
            ),
        )
    except EconomicsApplyError as exc:
    ...
    return {
        "ok": bool(
            missing_error == "economics_activation_preconditions_not_satisfied"
            and report.get("ready") is True
            and transfer_error == "economics_disabled"
            and ready_state.get("params", {}).get("economics_enabled") is False
        ),
        "batch": "560",
        "strict_preconditions_missing_error": missing_error,
        "activation_precondition_report": report,
        "wallet_initialization_policy_present": bool(
            report.get("checks", {}).get("wallet_initialization_policy_present")
        ),
        "reward_recipient_eligibility_present": bool(
            report.get("checks", {}).get("reward_recipient_eligibility_present")
        ),
        "anti_farming_policy_present": bool(
            report.get("checks", {}).get("anti_farming_policy_present")
        ),
        "transfer_receipt_policy_present": bool(
            report.get("checks", {}).get("transfer_receipt_policy_present")
        ),
        "treasury_accountability_policy_present": bool(
            report.get("checks", {}).get("treasury_accountability_policy_present")
        ),
