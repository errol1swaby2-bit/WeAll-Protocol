...
def _init_db(path: Path) -> sqlite3.Connection:
    ...
    con.execute(
        "CREATE TABLE IF NOT EXISTS blocks(height INTEGER PRIMARY KEY, block_id TEXT NOT NULL, parent_block_id TEXT NOT NULL, block_hash TEXT NOT NULL, state_root_after TEXT NOT NULL, receipt_root TEXT NOT NULL, json TEXT NOT NULL)"
    )
    con.execute(
        "CREATE TABLE IF NOT EXISTS replay_state(id INTEGER PRIMARY KEY CHECK(id=1), height INTEGER NOT NULL, state_root TEXT NOT NULL, json TEXT NOT NULL)"
    )
    con.commit()
    return con
...
def _make_source_blocks() -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    parent_id = "genesis"
    prev_hash = "genesis"
    for height, actor in enumerate(
        ("@alice", "@bob", "@carol", "@dave", "@erin", "@frank"), start=1
    ):
        ...
        if height <= 3:
            ...
            state["content"]["posts"][f"post:{height}"] = {
...
def _write_blocks(con: sqlite3.Connection, blocks: list[dict[str, Any]]) -> None:
    for block in blocks:
        con.execute(
            "INSERT OR REPLACE INTO blocks(height, block_id, parent_block_id, block_hash, state_root_after, receipt_root, json) VALUES (?, ?, ?, ?, ?, ?, ?)",
            (
                int(block["height"]),
                str(block["block_id"]),
                str(block["parent_block_id"]),
                str(block["block_hash"]),
                str(block["state_root_after"]),
                str(block["receipt_root"]),
...
def _read_blocks(con: sqlite3.Connection, *, after_height: int = 0) -> list[dict[str, Any]]:
    rows = con.execute(
        "SELECT json FROM blocks WHERE height > ? ORDER BY height ASC", (int(after_height),)
    ).fetchall()
    return [json.loads(row[0]) for row in rows]


def _validate_block(
    block: dict[str, Any], *, expected_height: int, parent_id: str, prev_hash: str
) -> None:
    ...
    if str(block.get("receipt_root") or "") != _receipt_root(
...
def _apply_block_state(state: dict[str, Any], block: dict[str, Any]) -> None:
    ...
    if height <= 3:
        ...
        state.setdefault("content", {}).setdefault("posts", {})[f"post:{height}"] = {
            "post_id": f"post:{height}",
            "author": actor,
            "created_at_nonce": height,
            "visibility": "public",
...
def _save_replay_state(con: sqlite3.Connection, state: dict[str, Any]) -> None:
    con.execute(
        "INSERT OR REPLACE INTO replay_state(id, height, state_root, json) VALUES (1, ?, ?, ?)",
        (
...
def _replay_into(
    con: sqlite3.Connection, blocks: list[dict[str, Any]], *, stop_after: int | None = None
) -> dict[str, Any]:
    ...
    if int(state.get("height") or 0) > 0:
        row = con.execute(
            "SELECT block_hash FROM blocks WHERE height=?", (int(state.get("height") or 0),)
        ).fetchone()
    ...
    for block in blocks:
        ...
        if stop_after is not None and int(block["height"]) > int(stop_after):
            break
        _validate_block(
            block, expected_height=expected_height, parent_id=parent_id, prev_hash=prev_hash
        )
...
def run_harness() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="weall-db-replay-") as td:
        ...
        try:
            ...
            resumed = _replay_into(
                fresh, _read_blocks(fresh, after_height=int(partial.get("height") or 0))
            )
