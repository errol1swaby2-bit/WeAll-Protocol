...
def build() -> Json:
    source_checks = {
        "node_dashboard_public_seed_surface": _contains(
            "web/src/pages/NodeDashboard.tsx", "Seed, validator, and tx propagation visibility"
        ),
        "node_dashboard_validator_freshness_surface": _contains(
            "web/src/pages/NodeDashboard.tsx", "Fresh validator endpoints"
        ),
        "node_dashboard_nat_recovery_surface": _contains(
            "web/src/pages/NodeDashboard.tsx", "Peer / NAT recovery"
        ),
        "node_dashboard_promotion_path_surface": _contains(
            "web/src/pages/NodeDashboard.tsx", "Validator promotion path"
        ),
        "validator_wizard_backend_derived": _contains(
            "web/src/components/ValidatorReadinessWizard.tsx", "Backend-derived"
        ),
        "tx_lifecycle_component_present": _contains(
            "web/src/components/TxPropagationTimeline.tsx", "Propagation lifecycle"
        ),
        "tx_page_uses_lifecycle_component": _contains(
            "web/src/pages/TransactionsPage.tsx", "TxPropagationTimeline"
        ),
        "tx_lifecycle_upstream_acceptance_label": _contains(
            "web/src/pages/TransactionsPage.tsx", "Upstream validator accepted"
        ),
        "rendered_public_observer_spec_present": (
            REPO / "web" / "tests" / "e2e" / "public_observer_dashboard.spec.ts"
        ).is_file(),
        "package_script_public_observer_rendered_present": _contains(
            "web/package.json", "test:public-observer-rendered"
        ),
    }
    required_surfaces = [
    ...
    payload["artifact_digest"] = hashlib.sha256(
        _canon({"source_checks": source_checks, "required_surfaces": required_surfaces}).encode(
            "utf-8"
        )
    ).hexdigest()
    return payload
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate/check public frontend operator journey evidence scaffold."
    )
    ...
    if args.check:
        ...
        print(
            f"OK: {OUT.relative_to(ROOT)} is current ({len(payload['required_surfaces'])} surfaces)"
        )
        return 0 if payload.get("ok") else 1
