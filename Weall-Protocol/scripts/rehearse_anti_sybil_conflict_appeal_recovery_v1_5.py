...
def run_harness() -> Json:
    state: Json = {
        "height": 900,
        "accounts": {f"reviewer-{c}": {"poh_tier": 2} for c in "abcdefg"}
        | {"subject": {"poh_tier": 2}},
        "poh": {
            ...
            "reviewer_history_scores": {
                "reviewer-a": {
                    ...
                    "eligible": False,
                    "confirmed_collusion_count": 1,
                    "suspended_until_height": 5000,
                },
        ...
        "roles": {
            "poh_reviewers": {
                "active": {"reviewer-a": False},
                "suspended": {"reviewer-a": {"reason": "test"}},
            }
        },
    }
    ...
    deletion = execute_poh_evidence_deletion(
        state, challenge_id="challenge", reason="appeal_remedy_and_retention_completed"
    )
    selected = set(panel.get("selected_reviewers", []))
    excluded = set(panel.get("excluded_reviewers", []))
    return {
        "ok": signals["requires_adjudication_panel"] is True
        and panel["conflict_free_panel"] is True
        and confirmed["status"] == "adjudicated_confirmed"
        and appeal["recovery_applied"] is True
        and deletion["deleted"] is True,
