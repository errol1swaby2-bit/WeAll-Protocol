...
R20_STATE_KEY = "Content:account_id"
R20_STATE_ID = "STATE-47E618BD7C1C1376"
...
def register_r20_state_id() -> None:
    expected_suffix = hashlib.sha256(R20_STATE_KEY.encode("utf-8")).hexdigest()[:16].upper()
    if R20_STATE_ID != f"STATE-{expected_suffix}":
        raise SystemExit(
            f"r20 state stable-id derivation mismatch: {R20_STATE_ID} != STATE-{expected_suffix}"
        )
    ...
    existing = by_key.get(("state", R20_STATE_KEY))
    if existing is not None:
        if str(existing.get("stable_id") or "") != R20_STATE_ID:
            raise SystemExit(
                f"r20 state key already registered to unexpected ID: {existing.get('stable_id')}"
            )
        print(f"stable ID already registered: {R20_STATE_KEY} -> {R20_STATE_ID}")
        return

    collision = by_id.get(R20_STATE_ID)
    if collision is not None:
        raise SystemExit(
            "r20 deterministic state ID collision: "
            f"{R20_STATE_ID} already belongs to "
    ...
    entries.append(
        {
            "aliases": [],
            "canonical_key": R20_STATE_KEY,
            "kind": "state",
            "stable_id": R20_STATE_ID,
            "status": "active",
        }
    )
    ...
    print(f"registered stable ID: {R20_STATE_KEY} -> {R20_STATE_ID}")
