...
def _run_worker(
    db_path: Path, operator: str, url: str, *, max_attempts: int = 1
) -> tuple[IpfsPinWorker, dict[str, Any]]:
...
def run_harness() -> dict[str, Any]:
    ...
    try:
        ...
        with (
            tempfile.TemporaryDirectory(prefix="weall-b546-live-ipfs-worker-") as td,
            _ipfs_api(500) as failing,
            _ipfs_api(200) as success,
        ):
            root = Path(td)
            ...
            state: dict[str, Any] = {
                ...
                "storage": {"config": {"replication_factor": 1}},
            }
            for op in ("op-a", "op-b", "op-c"):
                ...
                apply_storage(
                    ...
                    _env(
                        ...
                        op,
                        10 + len(op),
                        {"offer_id": f"offer-{op}", "operator_id": op, "capacity_bytes": 4096},
                    ),
                )
            req = apply_storage(
                state,
                _env(
                    ...
                    "@alice",
                    1,
                    {"pin_id": "pin-b546", "cid": CID, "size_bytes": 32, "replication_factor": 1},
                ),
            )
            targets = list(req.get("targets") or []) if isinstance(req, dict) else []
            failed_operator = targets[0] if targets else "op-a"
            failed_worker = IpfsPinWorker(
                IpfsPinWorkerConfig(
                    db_path=str(db_path),
                    operator_account=failed_operator,
                    ipfs_enabled=True,
                    ipfs_api_url=str(failing["url"]),
                    max_attempts=1,
                    request_timeout_s=1,
                )
            )
            enqueue = failed_worker.enqueue_job(
                CID, targets=[failed_operator], meta={"pin_id": "pin-b546"}
            )
            failed_stats = failed_worker.run_once()
            failure = apply_storage(
                state,
                _env(
                    "IPFS_PIN_CONFIRM",
                    "SYSTEM",
                    2,
                    {"pin_id": "pin-b546", "operator_id": failed_operator, "ok": False},
                    system=True,
                    parent="pin-b546",
                ),
            )
            ...
            replacement = str(
                pin_after_failure.get("latest_reassignment", {}).get("new_operator_id") or "op-b"
            )
            replacement_worker = IpfsPinWorker(
                IpfsPinWorkerConfig(
                    db_path=str(db_path),
                    operator_account=replacement,
                    ipfs_enabled=True,
                    ipfs_api_url=str(success["url"]),
                    max_attempts=1,
                    request_timeout_s=1,
                )
            )
            enqueue_replacement = replacement_worker.enqueue_job(
                CID,
                targets=[replacement],
                meta={"pin_id": "pin-b546", "replacement_for": failed_operator},
            )
            ...
            return {
                "ok": bool(
                    failed_stats.get("failed") == 1
                    and success_stats.get("pinned") == 1
                    and pin.get("availability_status") == "available"
                ),
                "batch": "546",
                ...
                "reassignment_recorded": bool(
                    pin_after_failure.get("latest_reassignment", {}).get("reassigned")
                ),
