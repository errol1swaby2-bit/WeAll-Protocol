...
def _collect_event_count(
    ...
    *,
    ...
    # Collect a required acknowledgement set within a bounded deadline.
    ...
    while len(matched) < expected:
        ...
        if remaining <= 0:
            break
        try:
            event = tx_queue.get(timeout=min(0.05, remaining))
        except queue.Empty:
            continue
...
def _process_context() -> mp.context.BaseContext:
    """Return the explicit, thread-safe process context used by this rehearsal."""
...
def run_harness() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="weall-b572-") as td:
        tx_queue: mp.Queue = process_context.Queue()
        input_queues: dict[str, mp.Queue] = {
            validator: process_context.Queue() for validator in VALIDATORS
        }
        files = {v: str(Path(td) / f"{v}.json") for v in VALIDATORS}
        procs = {
            v: process_context.Process(
                target=_node_loop, args=(v, input_queues[v], tx_queue, files[v]), daemon=True
            )
            for v in VALIDATORS
        }
        ...
        try:
            ...
            tx_seen = _collect_event_count(
                tx_queue,
                event_type="tx_seen",
                expected=len(VALIDATORS) * len(txs),
                timeout=2.0,
            )
            ...
            for height in range(1, 7):
                ...
                block = {
                    "height": height,
                    "proposer": leader,
                    "tx_ids": sorted(_h(t) for t in txs) if height == 1 else [],
                    "view": height,
                }
                ...
                votes = [
                    e
                    for e in _drain(tx_queue, timeout=0.4)
                    if e.get("type") == "vote" and e.get("block_id") == block["block_id"]
                ]
                ...
                acks = [
                    e
                    for e in _drain(tx_queue, timeout=0.4)
                    if e.get("type") == "commit_ack" and e.get("height") == height
                ]
                ...
                if height == 3:
                    ...
                    input_queues["v-d"].put({"type": "stop"})
                    procs["v-d"].join(timeout=1.0)
                    input_queues["v-d"] = process_context.Queue()
                    procs["v-d"] = process_context.Process(
                        target=_node_loop,
                        args=("v-d", input_queues["v-d"], tx_queue, files["v-d"]),
                        daemon=True,
                    )
                    procs["v-d"].start()
                    time.sleep(0.1)
                    input_queues["v-d"].put({"type": "catchup", "blocks": committed_blocks})
            ...
            return {
                ...
                and set(heights.values()) == {6}
                and len(tx_seen) == len(VALIDATORS) * len(txs),
                "batch": "572",
                "process_model": "independent_multiprocessing_validator_nodes",
                "multiprocessing_start_method": process_context.get_start_method(),
