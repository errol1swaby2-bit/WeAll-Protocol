    ...
    return {
        ...
        "accounts": {
            "@alice": {
                ...
                "banned": False,
                "locked": False,
                "session_keys": {"sk:@alice": {"active": True}},
            },
            "@bob": {
                "nonce": 0,
                "poh_tier": 2,
                ...
                "banned": False,
                "locked": False,
                "session_keys": {"sk:@bob": {"active": True}},
            },
            "@juror": {
                "nonce": 0,
                "poh_tier": 2,
...
def _run_poh_challenge_reverify(state: dict[str, Any]) -> dict[str, Any]:
    ...
    apply_poh(
        ...
        _env(
            ...
            1,
            {
                "case_id": case_id,
                "account_id": "@alice",
                "evidence_commitment": "sha256:" + "a" * 64,
            },
        ),
    )
    apply_poh(
        ...
        _env(
            ...
            "@alice",
            2,
            {"case_id": case_id, "evidence_id": "ev1", "evidence_commitment": "sha256:" + "a" * 64},
        ),
    )
    apply_poh(
        state,
        _env(
            ...
            "SYSTEM",
            1,
            {"case_id": case_id, "jurors": ["@reviewer", "@bob", "@juror"]},
            system=True,
            parent="poh:case",
        ),
    )
    ...
    apply_poh(
        state,
        _env(
            "POH_ASYNC_FINALIZE", "SYSTEM", 1, {"case_id": case_id}, system=True, parent="poh:case"
        ),
    )
    open_out = apply_poh(
        state,
        _env(
            ...
            "@bob",
            1,
            {"account_id": "@alice", "case_id": case_id, "reason": "duplicate-human-risk"},
        ),
    )
    ...
    resolved = apply_poh(
        ...
        _env(
            ...
            "SYSTEM",
            2,
            {"challenge_id": cid, "resolution": "upheld", "case_id": case_id},
            system=True,
    ...
    apply_poh(
        ...
        _env(
            ...
            3,
            {
                "case_id": rev_case,
                "account_id": "@alice",
                "evidence_commitment": "sha256:" + "b" * 64,
                "challenge_id": cid,
            },
        ),
    )
    ...
    apply_poh(
        ...
        _env(
            ...
            "SYSTEM",
            2,
            {"case_id": rev_case, "jurors": ["@bob", "@juror", "@reviewer"]},
            system=True,
            parent="poh:reverify",
        ),
    )
    for idx, juror in enumerate(["@bob", "@juror", "@reviewer"], start=20):
        ...
        apply_poh(
            ...
            _env(
                ...
                juror,
                idx + 10,
                {"case_id": rev_case, "verdict": "approve"},
            ),
        )
    finalized = apply_poh(
        state,
    ...
    return {
        "challenge_id": cid,
        "resolve": resolved,
        "reverify_finalize": finalized,
        "reviewer_eligible": state["accounts"]["@reviewer"].get("poh_reviewer_eligible"),
    }
...
def run_harness() -> dict[str, Any]:
    ...
    group = apply_groups(
        ...
        _env(
            ...
            "@bob",
            1,
            {"group_id": "g-life", "name": "Lifecycle", "visibility": "public"},
        ),
    )
    post = apply_content(
        state,
        _env(
            ...
            3,
            {
                "post_id": "post:lifecycle:api",
                "body": "v1.5 api lifecycle",
                "visibility": "public",
                "tags": ["v15"],
            },
        ),
    ...
    apply_content(
        ...
        _env(
            ...
            "@bob",
            3,
            {"target_id": "post:lifecycle:api", "reaction": "helpful"},
        ),
    )
    ...
    dispute = apply_dispute(
        ...
        _env(
            ...
            4,
            {
                "dispute_id": "d-api-life",
                "target_type": "content",
                "target_id": "post:lifecycle:api",
                "reason": "appealable lifecycle",
            },
        ),
    )
    apply_dispute(
        ...
        _env(
            ...
            "SYSTEM",
            4,
            {"dispute_id": "d-api-life", "juror": "@juror"},
            system=True,
            parent="dispute:open",
        ),
    )
    ...
    apply_dispute(
        ...
        _env(
            ...
            6,
            {
                ...
                "vote": "yes",
                "resolution": {
                    "summary": "temporary account posting limit",
                    "actions": [
                        {
                            "tx_type": "ACCOUNT_RESTRICTION_SET",
                            "payload": {"account_id": "@bob", "restriction": "posting_limited"},
                        }
                    ],
                },
            },
    ...
    failed_pin = apply_storage(
        ...
        _env(
            ...
            "SYSTEM",
            8,
            {"pin_id": "pin-api", "cid": CID_A, "operator_id": "opA", "ok": False},
            system=True,
    ...
    try:
        apply_economics(
            state, _env("BALANCE_TRANSFER", "@bob", 10, {"to": "@bob", "amount_int": 1})
        )
    except EconomicsApplyError as exc:
        econ_rejected = exc.reason in {"economics_disabled", "economics_time_locked"}
    declared = apply_protocol(
        state,
        _env(
            "PROTOCOL_UPGRADE_DECLARE",
            "SYSTEM",
            11,
            {"upgrade_id": "u-api-life", "version": "v1.5-api", "hash": "sha256:api"},
            system=True,
            parent="gov:upgrade",
        ),
    )
    activated = apply_protocol(
        state,
        _env(
            "PROTOCOL_UPGRADE_ACTIVATE",
            "SYSTEM",
            12,
            {"upgrade_id": "u-api-life"},
            system=True,
            parent="protocol:u-api-life",
        ),
    )
    ...
    active = (
        state.get("protocol", {}).get("active", {})
        if isinstance(state.get("protocol"), dict)
        else {}
    )
    return {
        "ok": bool(
            session_me.status_code == 200
            and feed.status_code == 200
            and activity.status_code < 500
            and feed.json().get("ranking", {}).get("mode") == "production"
            and public_activity_checked
            and final.get("enforcement_applied")
            and state.get("storage", {})
            .get("pins", {})
            .get("pin-api", {})
            .get("availability_status")
            == "available"
            and active.get("record_only_boundary", {}).get("artifact_apply_enabled") is False
        ),
        "batch": "530",
        ...
        "protocol_upgrade_record_only": bool(
            isinstance(active.get("record_only_boundary"), dict)
            and active["record_only_boundary"].get("artifact_apply_enabled") is False
        ),
