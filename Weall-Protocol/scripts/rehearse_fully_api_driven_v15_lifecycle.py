...
def _enable_storage_responsibility(
    state: dict[str, Any], operator_id: str, *, capacity: int = 4096
) -> None:
...
def run_harness() -> dict[str, Any]:
    state: dict[str, Any] = {
        ...
        "params": {
            ...
            "tokenomics_simulation_complete": True,
            "reward_policy_complete": True,
        },
        "system_queue": [],
    }
    api_routes: list[str] = []
    ...
    session = client.get(
        "/v1/session/me", headers={"x-weall-account": "@alice", "x-weall-session-key": "missing"}
    )
    ...
    apply_poh(
        ...
        _env(
            ...
            "SYSTEM",
            1,
            {"case_id": "case-a", "jurors": ["@reviewer", "@juror", "@r2"]},
            system=True,
            parent="poh:case",
        ),
    )
    ...
    apply_poh(
        state,
        _env(
            "POH_ASYNC_FINALIZE", "SYSTEM", 2, {"case_id": "case-a"}, system=True, parent="poh:case"
        ),
    )
    challenge = apply_poh(
        state, _env("POH_CHALLENGE_OPEN", "@bob", 3, {"account_id": "@alice", "case_id": "case-a"})
    )
    apply_poh(
        state,
        _env(
    ...
    apply_poh(
        ...
        _env(
            ...
            "SYSTEM",
            5,
            {"case_id": "case-r", "jurors": ["@reviewer", "@juror", "@r2"]},
            system=True,
            parent="poh:case",
        ),
    )
    for idx, reviewer in enumerate(["@reviewer", "@juror", "@r2"], start=1):
        ...
        apply_poh(
            ...
            _env(
                ...
                reviewer,
                40 + idx,
                {"case_id": "case-r", "verdict": "approve"},
            ),
        )
    apply_poh(
        state,
        _env(
            "POH_ASYNC_FINALIZE", "SYSTEM", 6, {"case_id": "case-r"}, system=True, parent="poh:case"
        ),
    )
    ...
    apply_groups(
        ...
        _env(
            ...
            "@alice",
            50,
            {"group_id": "g1", "name": "Group One", "visibility": "public"},
        ),
    )
    ...
    apply_content(
        state, _env("CONTENT_REACTION_SET", "@bob", 53, {"target_id": "post:1", "reaction": "like"})
    )
    apply_content(
        state,
        _env(
            ...
            "@alice",
            54,
            {"comment_id": "comment:1", "post_id": "post:1", "body": "public author follow-up"},
        ),
    )
    ...
    activity = client.get(
        "/v1/activity/notices",
        headers={"x-weall-account": "@alice", "x-weall-session-key": "missing"},
    )
    ...
    apply_dispute(
        ...
        _env(
            ...
            "@bob",
            60,
            {"dispute_id": "d1", "target_type": "account", "target_id": "@alice", "reason": "test"},
        ),
    )
    apply_dispute(
        state,
                    ...
                    "actions": [
                        {
                            ...
                            "payload": {"account_id": "@alice", "restriction": "review"},
                        }
                    ],
                },
            },
                    ...
                    "actions": [
                        {
                            ...
                            "payload": {"account_id": "@alice", "reason": "appeal_remedy"},
                        },
                        {
    ...
    pin = apply_storage(
        ...
        _env(
            ...
            "SYSTEM",
            72,
            {"pin_id": "pin-1", "cid": cid, "size_bytes": 10, "replication_factor": 1},
            system=True,
            parent="storage",
        ),
    )
    ...
    try:
        ...
        econ_locked = True
    proto = apply_protocol(
        state,
        _env(
            "PROTOCOL_UPGRADE_DECLARE",
            "SYSTEM",
            90,
            {"upgrade_id": "u1", "version": "v1.5.x"},
            system=True,
            parent="gov",
        ),
    )
    ...
    return {
        "ok": all(
            r.status_code < 500 for r in [session, feed, group, group_feed, activity, dispute_read]
        )
        and econ_locked
        and bool(proto.get("record_only_boundary"))
        and bool(receipt.get("enforcement_applied")),
        "batch": "536",
        ...
        "poh_reverification_status": state["poh"]["reverification"]["by_account"]["@alice"][
            "status"
        ],
        "dispute_remedy_applied": any(
            a.get("tx_type") == "ACCOUNT_REINSTATE" for a in receipt.get("enforcement_applied", [])
        ),
        "storage_retrieval_confirmed": state["storage"]["pins"]["pin-1"].get("durability_status")
        == "retrieval_confirmed",
        "economics_locked_rejection": econ_locked,
        "protocol_upgrade_record_only": bool(
            state.get("protocol", {}).get("upgrades", {}).get("u1", {}).get("record_only_boundary")
        ),
