...
def _init_node_db(path: Path) -> sqlite3.Connection:
    ...
    con.execute(
        "CREATE TABLE IF NOT EXISTS blocks(height INTEGER PRIMARY KEY, block_id TEXT NOT NULL, block_hash TEXT NOT NULL, block_json TEXT NOT NULL)"
    )
    con.execute(
        "CREATE TABLE IF NOT EXISTS block_hash_index(block_id TEXT PRIMARY KEY, block_hash TEXT NOT NULL, height INTEGER NOT NULL)"
    )
    con.execute(
        "CREATE TABLE IF NOT EXISTS ledger_state(id INTEGER PRIMARY KEY CHECK(id=1), height INTEGER NOT NULL, block_id TEXT NOT NULL, state_json TEXT NOT NULL)"
    )
    con.commit()
    return con
...
def _make_block(
    height: int, parent_id: str, prev_hash: str, state: dict[str, Any], tx_id: str
) -> dict[str, Any]:
...
def _insert_committed(
    con: sqlite3.Connection, block: dict[str, Any], state: dict[str, Any]
) -> None:
    con.execute(
        "INSERT INTO blocks(height, block_id, block_hash, block_json) VALUES (?, ?, ?, ?)",
        (int(block["height"]), str(block["block_id"]), str(block["block_hash"]), _canon(block)),
    )
    con.execute(
        "INSERT INTO block_hash_index(block_id, block_hash, height) VALUES (?, ?, ?)",
        (str(block["block_id"]), str(block["block_hash"]), int(block["height"])),
    )
    con.execute(
        "INSERT OR REPLACE INTO ledger_state(id, height, block_id, state_json) VALUES (1, ?, ?, ?)",
        (int(state["height"]), str(block["block_id"]), _canon(state)),
    )
    con.commit()
...
def _build_source(con: sqlite3.Connection) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    for height in range(1, 8):
        if height <= 3:
            ...
            state["content"]["posts"][post_id] = {
                "post_id": post_id,
                "author": f"@user{height - 3}",
                "created_at_nonce": height,
                "visibility": "public",
            }
    ...
    con.execute(
        "INSERT OR REPLACE INTO ledger_state(id, height, block_id, state_json) VALUES (1, ?, ?, ?)",
        (int(state["height"]), str(state["finalized"]["block_id"]), _canon(state)),
    )
    con.commit()
    return state, blocks
...
def _read_blocks(con: sqlite3.Connection, after_height: int = 0) -> list[dict[str, Any]]:
    rows = con.execute(
        "SELECT block_json FROM blocks WHERE height > ? ORDER BY height ASC", (int(after_height),)
    ).fetchall()
...
def _validate_block(
    block: dict[str, Any], *, expected_height: int, parent: str, prev_hash: str
) -> None:
    ...
    receipts = [
        r
        for r in (block.get("receipts") if isinstance(block.get("receipts"), list) else [])
    ...
    expected_hash = _hash(
        {
            ...
            "prev_block_hash": prev_hash,
            "receipt_root": str(block["receipt_root"]),
            "state_root_after": str(block["state_root_after"]),
        }
    )
...
def _apply_block(state: dict[str, Any], block: dict[str, Any]) -> None:
    ...
    if height <= 3:
        ...
        post_id = f"post:{height}"
        state.setdefault("content", {}).setdefault("posts", {})[post_id] = {
            "post_id": post_id,
            "author": f"@user{height - 3}",
            "created_at_nonce": height,
            "visibility": "public",
...
def _replay_from_source(
    source: sqlite3.Connection, fresh: sqlite3.Connection, *, stop_after: int | None = None
) -> dict[str, Any]:
    state = _load_state(fresh)
    start_height = int(state.get("height") or 0)
    parent = (
        str(state.get("finalized", {}).get("block_id") or "genesis")
        if isinstance(state.get("finalized"), dict)
        else "genesis"
    )
    if start_height:
        row = fresh.execute(
            "SELECT block_hash FROM blocks WHERE height=?", (start_height,)
        ).fetchone()
