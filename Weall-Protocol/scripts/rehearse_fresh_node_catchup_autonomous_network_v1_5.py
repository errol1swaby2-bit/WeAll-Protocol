...
def run_harness() -> dict[str, Any]:
    ...
    tx_ids = sorted(
        [
            _h({"signer": "@alice", "nonce": 1}),
            _h({"signer": "@bob", "nonce": 1}),
            _h({"signer": "@carol", "nonce": 1}),
        ]
    )
    block1_base = {"height": 1, "tx_ids": tx_ids, "proposer": "v-a", "partition": "majority"}
    block1_base["block_id"] = _h(block1_base)
    root1 = _h(
        {"prev": "genesis", "height": 1, "tx_ids": tx_ids, "block_id": block1_base["block_id"]}
    )
    block1 = {
        **block1_base,
        "state_root": root1,
        "receipt_root": _h({"height": 1, "tx_ids": tx_ids}),
    }
    ...
    try:
        bad = dict(live_peer_blocks[1])
        bad["block_id"] = "wrong-chain-block"
        _verify_block(bad, fresh_root)
    except Exception:
        wrong_chain_rejected = True
    corrupt_receipt_rejected = False
    try:
        bad = dict(live_peer_blocks[0])
        bad["state_root"] = "0" * 64
    ...
    return {
        "ok": bool(
            net.get("ok")
            and all(applied)
            and resumed_root == root2
            and wrong_chain_rejected
            and corrupt_receipt_rejected
        ),
        "batch": "568",
