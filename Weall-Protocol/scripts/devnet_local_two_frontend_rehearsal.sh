...
OBSERVER_ACCOUNT="${WEALL_OBSERVER_TEST_ACCOUNT:-@observer-user}"
GENESIS_ACCOUNT="${WEALL_GENESIS_BOOTSTRAP_ACCOUNT:-@devnet-genesis}"
OBSERVER_KEYFILE="${WEALL_OBSERVER_TEST_KEYFILE:-${DEVNET_DIR}/accounts/observer-user.json}"
...
if ! curl -fsS "${NODE2_API}/v1/status" >/dev/null 2>&1; then
  ...
  (
    ...
    export WEALL_TX_UPSTREAM_SYNC_ON_SUBMIT=1
    export WEALL_OBSERVER_READ_THROUGH_SYNC=1
...
if [[ -z "${DOWNSTREAM_SYNC_PID}" ]]; then
  ...
  (
    ...
    export WEALL_STATE_SYNC_OPERATOR_TOKEN="${SYNC_TOKEN}"
    export WEALL_OBSERVER_EDGE_OPERATOR_TOKEN="${OBSERVER_TOKEN}"
    export WEALL_DEVNET_SYNC_MAX_ROUNDS="${WEALL_LOCAL_REHEARSAL_DOWNSTREAM_SYNC_MAX_ROUNDS:-8}"
