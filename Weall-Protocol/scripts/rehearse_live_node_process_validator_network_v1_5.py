            ...
            try:
                ...
                if typ == "ping":
                    ...
                    if role != "validator" or node_id not in VALIDATORS:
                        ...
                        proposal = (
                            msg.get("proposal") if isinstance(msg.get("proposal"), dict) else {}
                        )
                        payload = {
                            "node_id": node_id,
                        ...
                        out = {
                            "ok": True,
                            "node_id": node_id,
                            "vote": {**payload, "vote_hash": _hash(payload)},
                        }
                elif typ == "commit":
                    ...
                    if role != "validator":
                        ...
                        out = {
                            "ok": False,
                            "node_id": node_id,
                            "error": "finality_threshold_not_met",
                        }
                    else:
                        committed = (
                            st.get("committed_blocks")
                            if isinstance(st.get("committed_blocks"), list)
                            else []
                        )
                        if not any(
                            isinstance(b, dict) and b.get("block_id") == block.get("block_id")
                            for b in committed
                        ):
                        ...
                        out = {
                            ...
                            "node_id": node_id,
                            "height": st["height"],
                            "root": _root_without_ephemeral_node_id(st),
                        }
...
def _wait_node_ready(
    ...
    *,
    ...
    while time.monotonic() < deadline:
        ...
        try:
            reply = _rpc(port, {"type": "ping"}, timeout=0.5)
            if reply.get("ok"):
                return reply
            last = RuntimeError(f"ping_not_ok:{reply}")
        except Exception as exc:
            last = exc
        time.sleep(0.05)
...
def _start_node_ready(
    ...
    try:
        ready = _wait_node_ready(proc, port, node_id)
    except Exception:
...
def _commit_round(
    ...
    *,
    ...
    commit_replies = [
        _rpc(ports[vid], {"type": "commit", "block": block, "votes": votes}) for vid in VALIDATORS
    ]
    return {
...
def run_harness() -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="weall-live-validator-") as td:
        ...
        for vid in VALIDATORS:
            port = _free_port()
            ports[vid] = port
            proc, _ = _start_node_ready(root, vid, port)
            procs[vid] = proc
        try:
            r1 = _commit_round(ports, height=1, view=0, tx_ids=["tx:bootstrap"])
            r2 = _commit_round(
                ports, height=2, view=1, parent_id=r1["block"]["block_id"], tx_ids=["tx:gov"]
            )
            roots_before = {
                vid: _rpc(port, {"type": "snapshot"})["root"] for vid, port in ports.items()
            }
            ...
            minority = _commit_round(
                ...
                participants=VALIDATORS[:2],
                tx_ids=["tx:minority"],
            )
            reference_snapshot = _rpc(ports["validator-a"], {"type": "snapshot"})
            lag_port = _free_port()
            lag_proc, _ = _start_node_ready(root, "validator-lagging", lag_port)
            try:
                sync = _rpc(
                    lag_port,
                    {
                        "type": "sync_blocks",
                        "blocks": reference_snapshot["state"].get("committed_blocks", []),
                    },
                )
            finally:
                try:
                    ...
                    pass
                lag_proc.terminate()
                lag_proc.wait(timeout=3)
            obs_port = _free_port()
            obs_proc, _ = _start_node_ready(root, "observer-1", obs_port, role="observer")
            try:
                observer_vote = _rpc(
                    obs_port, {"type": "vote", "proposal": _proposal(9, 0, "genesis", [])}
                )
            finally:
                try:
                    ...
                    pass
                obs_proc.terminate()
                obs_proc.wait(timeout=3)
            roots_after = {
                vid: _rpc(port, {"type": "snapshot"})["root"] for vid, port in ports.items()
            }
        finally:
            for _vid, port in ports.items():
                try:
        ...
        ok = bool(
            r1.get("committed")
            and r2.get("committed")
            and len(set(roots_before.values())) == 1
            and restarted_root == roots_before["validator-d"]
            and minority.get("committed") is False
            and sync.get("root") == reference_snapshot.get("root")
            and observer_vote.get("ok") is False
