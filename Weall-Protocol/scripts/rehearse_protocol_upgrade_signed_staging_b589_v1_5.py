...
def _sign(payload_hash: str, signer: str = _SIGNER_ID) -> str:
    return hashlib.sha256(
        _SECRET + signer.encode("utf-8") + payload_hash.encode("utf-8")
    ).hexdigest()
...
def _verify(manifest: Json, allowed_signers: set[str]) -> tuple[bool, str]:
    ...
    body = {
        k: v for k, v in manifest.items() if k not in {"signature", "artifact_hash", "signer_id"}
    }
...
def run_harness() -> Json:
    body = {
        ...
        "rollback_vector_hash": hashlib.sha256(
            b"rollback plan documented but not executed"
        ).hexdigest(),
    ...
    auto_apply = {
        **auto_body,
        "signer_id": _SIGNER_ID,
        "artifact_hash": auto_hash,
        "signature": _sign(auto_hash),
    }
    ...
    return {
        "ok": bool(
            ...
            and not wrong_ok
            and not auto_ok
            and all(v is False for v in no_side_effects.values())
        ),
        "batch": "589",
