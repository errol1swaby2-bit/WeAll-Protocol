...
from weall.runtime.apply.economics import (
    economics_locked_long_run_stress_summary,
...
def run_harness() -> dict:
    ...
    claim = record_locked_reward_claim(
        state,
        claim_id="reward-stress-1",
        account_id="alice",
        epoch=24,
        amount=1,
        status="pending",
        reason="locked_read_model",
    )
    report = record_treasury_report(
        state,
        report_id="treasury-24",
        period="epochs-1-24",
        opening_balance=0,
        closing_balance=0,
        spends=[],
    ...
    ok = bool(
        not stress["economics_enabled"]
        and not stress["live_mutation_enabled"]
        and stress["rejected_claim_count"] > 0
        and failed["status"] == "failed"
        and read["live_mutation_enabled"] is False
    )
    return {
        "ok": ok,
        "batch": "576",
