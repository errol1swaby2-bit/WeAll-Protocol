...
def _make_local_executor(
    root: Path, name: str, chain_id: str, pubs: dict[str, str]
) -> WeAllExecutor:
    ex = WeAllExecutor(
...
def _produce_and_apply(
    leader, follower, pubs: dict[str, str], privs: dict[str, str], *, tx_account: str, nonce: int
) -> dict[str, Any]:
...
def _apply_blocks(
    executor, blocks: list[dict[str, Any]], pubs: dict[str, str], privs: dict[str, str]
) -> list[bool]:
    results: list[bool] = []
    for block in blocks:
...
def run_harness() -> dict[str, Any]:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="weall-b563-follower-sync-") as td:
            ...
            leader._ledger_store.write(leader.state)
            leader.state = leader._ledger_store.read()
            follower._ledger_store.write(follower.state)
            follower.state = follower._ledger_store.read()
            committed: list[dict[str, Any]] = []
            for idx in range(1, 4):
                committed.append(
                    _produce_and_apply(
                        leader, follower, pubs, privs, tx_account=f"@catchup-{idx}", nonce=1
                    )
                )
            ...
            fresh._ledger_store.write(fresh.state)
            fresh.state = fresh._ledger_store.read()
            first_req = StateSyncRequestMsg(
                header=_header(chain_id, "b563-first"),
                mode="delta",
                from_height=0,
                to_height=1,
            ...
            resumed._ledger_store.write(resumed.state)
            resumed.state = resumed._ledger_store.read()
            resume_req = StateSyncRequestMsg(
                header=_header(chain_id, "b563-resume"),
                mode="delta",
                from_height=interrupted_height,
                to_height=follower_height,
                selector={"trusted_anchor": anchor},
            )
            ...
            return {
                "ok": bool(
                    follower_height == len(committed)
                    and all(first_results)
                    and all(resume_results)
                    and fresh_root == follower_root
                ),
                "batch": "563",
