...
def build() -> Json:
    checks = {
        ...
        "runtime_requires_pinned_signer": _contains(
            "src/weall/api/public_seed_registry.py", "public_seed_registry_signer_pin_missing"
        ),
        "runtime_rejects_unpinned_signer": _contains(
            "src/weall/api/public_seed_registry.py", "public_seed_registry_unpinned_signer"
        ),
        "signing_script_present": (
            ROOT / "scripts" / "sign_public_seed_registry_v1_5.py"
        ).is_file(),
        "placeholder_gate_present": _contains(
            "src/weall/api/public_seed_registry.py", "public_seed_registry_placeholder_"
        ),
    }
    operations = [
    ...
    payload["artifact_digest"] = hashlib.sha256(
        _canon({"checks": checks, "operations": operations}).encode("utf-8")
    ).hexdigest()
    return payload
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate/check public registry signer operations artifact."
    )
    ...
    if args.check:
        if not OUT.exists() or OUT.read_text(encoding="utf-8") != text:
            raise SystemExit(
                "public_registry_signer_operations_v1_5.json is stale; rerun generator"
            )
        print(
            f"OK: {OUT.relative_to(ROOT)} is current ({len(payload['required_operations'])} operations)"
        )
        return 0 if payload.get("ok") else 1
