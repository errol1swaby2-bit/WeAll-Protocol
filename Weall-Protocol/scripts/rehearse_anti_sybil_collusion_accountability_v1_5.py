...
def _seed_case(
    state: dict[str, Any], *, case_id: str, subject: str, reviewers: list[str], commitment: str
) -> None:
    ...
    apply_poh(
        ...
        _env(
            ...
            subject,
            1,
            {"case_id": case_id, "account_id": subject, "evidence_commitment": commitment},
        ),
    )
    apply_poh(
        state,
        _env(
            ...
            subject,
            2,
            {"case_id": case_id, "evidence_id": "ev-" + case_id, "evidence_commitment": commitment},
        ),
    )
    apply_poh(
        state,
        _env(
            ...
            "SYSTEM",
            1,
            {"case_id": case_id, "jurors": reviewers},
            system=True,
            parent="poh",
        ),
    )
    ...
    apply_poh(
        state,
        _env("POH_ASYNC_FINALIZE", "SYSTEM", 2, {"case_id": case_id}, system=True, parent="poh"),
    )
...
def run_harness() -> dict[str, Any]:
    ...
    state: dict[str, Any] = {
        ...
        "accounts": {
            "@subject": {"poh_tier": 1},
            "@challenger": {"poh_tier": 1},
            **{r: {"poh_tier": 2} for r in reviewers},
        },
        "roles": {"poh_reviewers": {"active": {r: True for r in reviewers}}},
    }
    _seed_case(
        state,
        case_id="case-collusion",
        subject="@subject",
        reviewers=reviewers,
    ...
    suspicion = (
        next(iter(suspicions.values()), {}) if isinstance(suspicions, dict) and suspicions else {}
    )
    ...
    _seed_case(
        state,
        case_id="case-reverified",
        subject="@subject",
        reviewers=reviewers,
        commitment="sha256:" + "f" * 64,
    )
    retention = (
        state.get("poh", {})
        .get("evidence_retention", {})
        .get("by_challenge", {})
        .get(opened["challenge_id"], {})
    )
    return {
        "ok": bool(
            resolved.get("resolution") == "upheld"
            and suspicion.get("requires_followup_review") is True
            and retention.get("deletion_eligible") is True
        ),
        "batch": "559",
        "challenge_id": opened["challenge_id"],
        "reviewer_count_flagged": len(
            resolved.get("consequence", {}).get("reviewer_accountability", {}).get("reviewers")
            or []
        ),
        ...
        "reverification_status": state.get("poh", {})
        .get("reverification", {})
        .get("by_account", {})
        .get("@subject", {})
        .get("status"),
