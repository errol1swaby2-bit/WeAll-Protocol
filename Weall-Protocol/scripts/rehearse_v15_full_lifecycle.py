    ...
    return {
        ...
        "accounts": {
            ...
            "@juror": {
                ...
                "banned": False,
                "locked": False,
                "session_keys": {"sk:@juror": {"active": True}},
            },
...
def _run_content_dispute_storage_economics_protocol(state: dict[str, Any]) -> dict[str, Any]:
    ...
    apply_dispute(
        ...
        _env(
            ...
            "SYSTEM",
            1,
            {"dispute_id": "d-life", "juror": "@juror"},
            system=True,
            parent="tx:dispute-open",
        ),
    )
    ...
    apply_dispute(
        ...
        _env(
            ...
            2,
            {
                ...
                "vote": "yes",
                "resolution": {
                    "summary": "limit creator",
                    "actions": [
                        {
                            "tx_type": "ACCOUNT_RESTRICTION_SET",
                            "payload": {"account_id": "@alice", "restriction": "posting_limited"},
                        }
                    ],
                },
            },
    ...
    state.setdefault("storage", {}).setdefault("pins", {})["pin-life"] = {
        "pin_id": "pin-life",
        "cid": CID_A,
        "targets": ["opA"],
        "size_bytes": 10,
    ...
    try:
        apply_economics(
            state, _env("BALANCE_TRANSFER", "@alice", 2, {"to": "@bob", "amount_int": 1})
        )
    ...
    declared = apply_protocol(
        ...
        _env(
            ...
            "SYSTEM",
            4,
            {"upgrade_id": "u-life", "version": "v1.5-life", "hash": "sha256:life"},
            system=True,
            parent="gov:life",
        ),
    )
    ...
    return {
        ...
        "storage_retrieval_confirmed": bool(
            state.get("storage", {}).get("pins", {}).get("pin-life", {}).get("availability_status")
            == "available"
        ),
        ...
        "protocol_upgrade_record_only": bool(
            ...
            and active.get("record_only_boundary", {}).get("migration_execution_enabled") is False
        ),
