...
def run_harness() -> dict[str, Any]:
    promoted_validator_mempool = _load_script(
        "rehearse_genesis_observer_promoted_validator_mempool_v1_5.py"
    ).run_harness()
    process_net = _load_script(
        "rehearse_independent_process_validator_network_v1_5.py"
    ).run_harness()
    ...
    external_observer["ok"] = all(
        bool(v.get("ok")) for v in external_observer.values() if isinstance(v, dict)
    )
    ...
    return {
        "ok": bool(promoted_validator_mempool.get("ok"))
        and bool(process_net.get("ok"))
