...
def _seed_case(state: dict[str, Any], case_id: str, account: str) -> None:
    ...
    apply_poh(
        ...
        _env(
            ...
            "SYSTEM",
            1,
            {"case_id": case_id, "jurors": ["@reviewer", "@juror", "@r2"]},
            system=True,
            parent="poh",
        ),
    )
...
def run_harness() -> dict[str, Any]:
    ...
    return {
        "ok": bool(
            retention_before.get("status") == "retain_until_reverification_or_appeal"
            and retention_after.get("status") == "remedy_completed_minimal_retention"
        ),
        "batch": "552",
        "challenge_id": opened["challenge_id"],
        "resolution": resolved.get("resolution"),
        "subject_status_after_upheld": state.get("poh_status", {})
        .get("by_account", {})
        .get("@alice", {})
        .get("status"),
        ...
        .get("@alice", {})
        .get("status"),
        "reviewer_accountability_recorded": bool(
            state.get("poh", {}).get("reviewer_accountability", {}).get("by_reviewer")
        ),
