    ...
    for canonical_key, stable_id in EXTRA_FAILURE_IDS.items():
        ...
        if existing is not None:
            if str(existing.get("stable_id") or "") != stable_id:
                raise SystemExit(
                    f"extra failure key already registered to unexpected ID: {canonical_key}: {existing.get('stable_id')}"
                )
            continue
        collision = by_id.get(stable_id)
        if collision is not None:
            raise SystemExit(
                f"extra failure ID collision: {stable_id} already belongs to {collision.get('kind')}:{collision.get('canonical_key')}"
            )
        row = {
            "aliases": [],
