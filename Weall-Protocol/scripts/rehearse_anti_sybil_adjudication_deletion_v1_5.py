...
def run_harness() -> dict[str, Any]:
    ...
    suspicion = next(
        iter(
            state.get("poh", {})
            .get("reviewer_collusion_suspicions", {})
            .get("by_case", {})
            .values()
        )
    )
    confirmed = record_poh_collusion_adjudication(
        ...
        decision="confirmed",
        adjudicator_panel_id="panel:anti-sybil:1",
    )
    ...
    state["height"] = 5000
    dismissed = record_poh_collusion_adjudication(
        state,
        suspicion_id=suspicion["suspicion_id"],
        decision="dismissed",
        adjudicator_panel_id="panel:appeal:1",
        note="false_positive_remedy",
    )
    ...
    deletion = execute_poh_evidence_deletion(
        state, challenge_id=opened["challenge_id"], reason="false_positive_remedy_completed"
    )
    final_retention = state["poh"]["evidence_retention"]["by_challenge"][opened["challenge_id"]]
    return {
        "ok": bool(
            resolved.get("resolution") == "upheld"
            and confirmed.get("status") == "adjudicated_confirmed"
            and dismissed.get("status") == "adjudicated_dismissed"
            and deletion.get("deleted") is True
            and all(scores_after_dismiss[r]["eligible"] for r in reviewers)
        ),
        "batch": "570",
        "challenge_id": opened["challenge_id"],
        "suspicion_id": suspicion["suspicion_id"],
