...
| Reviewer lifecycle surface | `/v1/status/testnet-capabilities` exposes protocol-upgrade, governance, dispute, and minimum civic-loop status boundaries. | UX / reviewer evidence | Added public claim-boundary fields consumed by Node Dashboard. | Rendered browser E2E for the entire civic loop across a fresh backend. |
