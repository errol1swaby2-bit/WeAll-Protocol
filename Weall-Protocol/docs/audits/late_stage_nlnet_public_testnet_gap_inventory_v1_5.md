...
| Performance | Sustained-load harness and local evidence path exist; a historical local 2350 TPS figure exists in earlier audit material, but it is not a current-tree performance claim and must not be reused without a fresh exact-commit benchmark. | Evidence packaging | Kept wording as local harness only. | Public network performance evidence and hardware/environment-specific transcripts. |
...
- Protocol-upgrade domain apply now requires a SYSTEM queue or receipt-only parent reference even when tests/tools bypass the dispatcher.
- Upgrade payload attempts to activate economics, transfers, rewards, or fees are recorded as ignored record-only requests and do not mutate economics state.
- Testnet capability and Node Dashboard surfaces now expose governance/dispute block-height lifecycle boundaries and the minimum reviewer civic loop map.
