...
If any historical document mentions direct messages, private messages, chat, inbox/outbox, encrypted payloads, or member-only readable groups, that wording is legacy/out-of-scope unless the surrounding text explicitly says it is unsupported, removed, disabled, or forbidden for the current public-testnet scope.
...
Private/direct/encrypted messaging is not part of the current public-testnet scope and must not be presented in current UI or public-testnet documentation as an active feature.
