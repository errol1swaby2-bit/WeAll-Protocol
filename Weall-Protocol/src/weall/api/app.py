...
async def _lifespan(app: FastAPI):
    ...
    if executor is not None:
        ...
        try:
            ...
            webrtc_signal_bridge_loop = start_webrtc_signal_bridge_autodrain()
            app.state.webrtc_signal_bridge_loop = webrtc_signal_bridge_loop
        except Exception:
            webrtc_signal_bridge_loop = None
    ...
    try:
        ...
        try:
            from weall.api.routes_public_parts.poh import stop_webrtc_signal_bridge_autodrain
...
def create_app(*, boot_runtime: bool) -> FastAPI:
    ...
    if boot_runtime:
        ...
        # Build the runtime before the broader operator bootstrap audit so
        # chain-derived runtime authority failures surface with their precise
        # fail-closed reason. This preserves the authority contract invariant:
        # if BFT/validator service is requested but the on-chain/lifecycle
        # authority contract does not make it effective, startup must fail with
        # api_runtime_authority_validator_not_effective instead of being masked
        # by adjacent operator-environment issues such as missing fetch sources.
