...
class RelaySpool:
        ...
        with self._connect() as con:
            con.execute(
                ...
                INSERT OR IGNORE INTO relay_messages(
                    ...
                    tx_index_hash, msg_type, envelope_json, created_ms, expires_at_ms
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    str(env["relay_id"]),
    ...
    def status(self, *, now_ms: int | None = None) -> Json:
        now = int(now_ms if now_ms is not None else _now_ms())
        self.prune(now_ms=now)
        with self._connect() as con:
        ...
        return {"messages_total": total, "by_type": by_type, "by_recipient": by_recipient}
