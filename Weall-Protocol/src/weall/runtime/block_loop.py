...
class BlockProducerLoop:
    ...
    def _run(self) -> None:
        ...
        while not self._stop.is_set():
            ...
            # SQLite maintenance (best-effort; non-consensus)
            try:
                if hasattr(self._executor, "sqlite_maintenance_tick"):
                    self._executor.sqlite_maintenance_tick()
            except Exception:
                inc_counter("block_loop_sqlite_maint_errors_total", 1)
                log.exception("sqlite maintenance failed")

            # DB retention/pruning (best-effort; non-consensus)
            try:
                if hasattr(self._executor, "prune_history_tick"):
                    self._executor.prune_history_tick()
            except Exception:
                inc_counter("block_loop_db_prune_errors_total", 1)
