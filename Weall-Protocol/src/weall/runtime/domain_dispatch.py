...
def _enforce_apply_time_canon(state: Json, env: Any) -> None:
    ...
    if system_enforced:
        ...
        if not system_flag:
            # Apply-time canon enforcement preserves the long-standing domain
            # error contract: a non-system attempt to apply a SYSTEM/system_only
            # transaction is reported as ``system_only``. Admission can still
            # use the more edge-specific ``system_flag_required`` reason, but
            # direct apply/replay paths should match domain appliers and legacy
            # tests.
            raise ApplyError("forbidden", "system_only", {"tx_type": t})
