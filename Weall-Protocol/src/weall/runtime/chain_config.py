...
class ChainConfig:
    ...
    chain_manifest_path: str = ""
    chain_manifest_hash: str = ""
    expected_genesis_hash: str = ""
    expected_genesis_state_root: str = ""
...
_ALLOWED_MODES = {"dev", "test", "testnet", "devnet", "controlled_devnet", "demo", "prod"}
...
def load_chain_config(path: str | None = None) -> ChainConfig:
    ...
    if resolved.is_file():
        ...
        if path or env_path or mode_hint == "prod":
    ...
    env_node_id = str(os.environ.get("WEALL_NODE_ID", "") or "").strip()
    env_db_path = str(os.environ.get("WEALL_DB_PATH", "") or "").strip()
    env_tx_index_path = str(os.environ.get("WEALL_TX_INDEX_PATH", "") or "").strip()
    env_api_host = str(os.environ.get("WEALL_API_HOST", "") or "").strip()
    env_api_port = str(os.environ.get("WEALL_API_PORT", "") or "").strip()
    env_log_level = str(os.environ.get("WEALL_LOG_LEVEL", "") or "").strip()

    cfg = ChainConfig(
        chain_id=_as_str(env_chain_id or payload.get("chain_id") or manifest_chain_id, "weall-dev"),
        ...
        api_port=_as_int(env_api_port or payload.get("api_port"), 8000),
        allow_unsigned_txs=_as_bool(payload.get("allow_unsigned_txs"), False),
        log_level=_as_str(env_log_level or payload.get("log_level"), "INFO"),
        chain_manifest_path=manifest.path if manifest is not None else "",
        chain_manifest_hash=manifest.manifest_hash if manifest is not None else "",
        expected_genesis_hash=manifest.genesis_hash if manifest is not None else "",
        expected_genesis_state_root=manifest.genesis_state_root if manifest is not None else "",
        expected_tx_index_hash=manifest.tx_index_hash if manifest is not None else "",
        protocol_profile=manifest_profile,
    )
    validate_chain_config(cfg)
    return cfg
