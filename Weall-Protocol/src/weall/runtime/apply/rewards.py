...
def _ensure_monetary_policy(state: Json) -> Json:
    """Ensure economics.monetary_policy exists.

    Monetary policy is updated by BLOCK_REWARD_MINT (issuance accounting).
    """
...
def _wrap_econ_gate(state: Json, tx_type: str) -> None:
    ...
    for db in debits:
        ...
        if strict_debits and bal < int(amt):
            raise RewardsApplyError(
                ...
                {"account_id": str(from_acct), "balance": int(bal), "debit": int(amt)},
            )
        new_bal = bal - int(amt)
        if new_bal < 0:
            new_bal = 0
