...
def prepare_executor_init_paths(
    *, db_path: str, tx_index_path: str, aux_db_path: str | None = None
    ...
    aux_db_override = explicit_aux_db_path or str(os.environ.get("WEALL_AUX_DB_PATH") or "").strip()
...
def boot_config_from_env() -> ExecutorBootConfig:
    ...
    if (
        mode == "prod"
        and not env_chain_id
        and not (explicit_manifest or explicit_manifest_required)
    ):
    ...
    if mode == "prod":
        if not env_chain_id and not (
            manifest_chain_id and (explicit_manifest or explicit_manifest_required)
        ):
            raise RuntimeError("Missing required env for production: WEALL_CHAIN_ID")
