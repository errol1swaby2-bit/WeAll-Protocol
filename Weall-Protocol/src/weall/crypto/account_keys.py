...
def account_key_record_from_payload(
    ...
    *,
    ...
    pubkey = str(
        pubkeys.get("mldsa") or payload.get("mldsa_pubkey") or payload.get("pubkey") or ""
    ).strip()
    return mldsa_account_key_record(
        pubkey=pubkey, created_height=created_height, active=True, key_type=key_type
    )
...
def account_key_pubkey(record: Any, *, preferred_profile: str = "") -> str:
    ...
    profile = normalize_signature_profile_id(
        record.get("sig_profile") or preferred_profile or PQ_MLDSA_V1
    )
    if profile != PQ_MLDSA_V1:
        return ""
