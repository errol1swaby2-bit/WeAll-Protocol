...
def test_group_emissary_election_window_enforced() -> None:
    ...
    with pytest.raises(GroupsApplyError) as e1:
        apply_groups(state, _env("GROUP_EMISSARY_BALLOT_CAST", "@bob", 10, {"election_id": "e-win", "ranking": ["@bob", "@alice"]}))
    assert e1.value.reason == "election_not_started"

    # On time: applying at height 105
    state["height"] = 104
    meta = apply_groups(state, _env("GROUP_EMISSARY_BALLOT_CAST", "@bob", 11, {"election_id": "e-win", "ranking": ["@bob", "@alice"]}))
    ...
    with pytest.raises(GroupsApplyError) as e2:
        apply_groups(state, _env("GROUP_EMISSARY_ELECTION_FINALIZE", "@alice", 20, {"election_id": "e-win"}))
    ...
    with pytest.raises(GroupsApplyError) as e3:
        apply_groups(state, _env("GROUP_EMISSARY_BALLOT_CAST", "@carl", 30, {"election_id": "e-win", "ranking": ["@carl", "@alice"]}))
    assert e3.value.reason == "election_closed"

    # Finalize on/after end: applying at height 110
    state["height"] = 109
    meta2 = apply_groups(state, _env("GROUP_EMISSARY_ELECTION_FINALIZE", "@alice", 40, {"election_id": "e-win"}))
