...
def test_bft_fetch_tick_recovers_missing_parent_and_replays_chain(tmp_path: Path, monkeypatch) -> None:
    ...
    loop = NetMeshLoop(executor=follower, mempool=follower._mempool)

    def fake_get_json(url: str, *, timeout_s: float = 2.0):
        assert str(block1["block_id"]) in url
        return {"ok": True, "block": block1}

    monkeypatch.setattr("weall.net.net_loop._http_get_json", fake_get_json)
...
def test_bft_fetch_tick_rejects_mismatched_fetched_block(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("WEALL_MODE", "testnet")
    monkeypatch.setenv("WEALL_BFT_ENABLED", "1")
    monkeypatch.setenv("WEALL_BFT_FETCH_BASE_URLS", "http://peer-a")
    ...
    assert follower.bft_on_proposal({"view": 2, "proposer": "v2", "block": block2}) is None
    want = follower.bft_pending_fetch_requests()
    assert want == [str(block1["block_id"])]
    ...
    assert follower.bft_pending_fetch_requests() == want
