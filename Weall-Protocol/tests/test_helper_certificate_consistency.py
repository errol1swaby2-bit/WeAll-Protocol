...
def test_helper_execution_certificate_internal_consistency_accepts_matching_tx_order_hash() -> None:
    ...
    repaired = HelperExecutionCertificate(
        **{**cert.to_json(), "tx_order_hash": cert.compute_tx_order_hash()}
    )
    assert repaired.verify_internal_consistency() is True


def test_helper_execution_certificate_internal_consistency_rejects_mismatched_tx_order_hash() -> (
    None
):
