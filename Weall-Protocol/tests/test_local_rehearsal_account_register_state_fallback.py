...
def test_local_rehearsal_account_register_uses_state_fallback_without_weakening_live() -> None:
    ...
    assert (
        "Observer account-registration tx status has not proven upstream confirmation yet." in src
    )
    assert "Falling back only for this setup account to state proof on both nodes." in src
    assert (
        "Live/async verification txs still require confirmed-and-synced tx/case visibility." in src
    )
    assert (
