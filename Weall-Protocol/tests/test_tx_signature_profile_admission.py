...
def test_closed_testnet_rejects_missing_sig_profile(monkeypatch):
    ...
    msg = canonical_tx_message(
        chain_id="weall-testnet-v1", tx_type="ACCOUNT_UPDATE", signer="@alice", nonce=1, payload={}
    )
    tx = {
        "chain_id": "weall-testnet-v1",
        "tx_type": "ACCOUNT_UPDATE",
        "signer": "@alice",
        "nonce": 1,
        "payload": {},
        "sig": sign_mldsa(message=msg, privkey=priv),
    }
...
def test_closed_testnet_rejects_removed_classical_profile(monkeypatch):
    ...
    msg = canonical_tx_message(
        chain_id="weall-testnet-v1",
        network_id="weall-public-observer-testnet-v1",
        sig_profile=REMOVED_CLASSICAL_PROFILE,
        tx_type="ACCOUNT_UPDATE",
        signer="@alice",
        nonce=1,
        payload={},
...
def test_canonical_tx_message_binds_chain_and_profile():
    a = canonical_tx_message(
        chain_id="c1", sig_profile=PQ_MLDSA_V1, tx_type="X", signer="@a", nonce=1, payload={}
    )
    b = canonical_tx_message(
        chain_id="c1",
        sig_profile=REMOVED_CLASSICAL_PROFILE,
        tx_type="X",
        signer="@a",
    ...
    c = canonical_tx_message(
        chain_id="c2", sig_profile=PQ_MLDSA_V1, tx_type="X", signer="@a", nonce=1, payload={}
    )
    assert a != b
    assert a != c
