...
def test_every_canon_tx_is_claimed_by_apply_tx(tx_type: str) -> None:
    ...
    claimed = _attempt_apply(tx_type, system_flag=False) or _attempt_apply(
        tx_type, system_flag=True
    )
...
def test_receipt_only_canon_txs_are_claimed_when_parent_present(txdef: dict[str, Any]) -> None:
    ...
    assert claimed, (
        f"Receipt-only canon tx not claimed by apply_tx() when parent is present: {tx_type}"
    )
