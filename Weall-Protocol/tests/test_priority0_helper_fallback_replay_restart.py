...
def test_helper_late_and_conflicting_replays_stay_rejected_after_restart_and_fallback(
    tmp_path,
) -> None:
    ...
    assert any(
        item.lane_id == fallback_lane.lane_id and item.code == "fallback_finalized"
    ...
    late_duplicate = guard2.ingest_certificate(
        cert=accepted_cert, peer_id=str(accepted_lane.helper_id)
    )
    ...
    conflicting = guard2.ingest_certificate(
        cert=conflicting_cert, peer_id=str(accepted_lane.helper_id)
    )
    ...
    late_fallback = guard2.ingest_certificate(
        cert=fallback_cert, peer_id=str(fallback_lane.helper_id or "peer-fallback")
    )
