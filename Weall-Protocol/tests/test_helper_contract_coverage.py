...
def test_every_canon_tx_has_helper_contract() -> None:
...
def test_helper_contracts_only_emit_known_effective_lanes() -> None:
    contract_map = build_helper_contract_map(TX_INDEX)
...
def test_parallel_contracts_require_parallel_planner_and_execution() -> None:
    contract_map = build_helper_contract_map(TX_INDEX)
    for item in contract_map["contracts"]:
...
def test_planner_parallel_execution_serial_mismatches_are_explicitly_fail_closed() -> None:
    contract_map = build_helper_contract_map(TX_INDEX)
