...
def test_email_oracle_contract_has_provider_neutral_routes() -> None:
    weall_root = _repo_root()
    backend_paths = _extract_backend_v1_paths(weall_root)

    assert "/v1/poh/email/begin" in backend_paths
    assert "/v1/poh/email/complete" in backend_paths
    assert "/v1/poh/email/tx/attestation-submit" in backend_paths
    assert "/v1/poh/email/tx/receipt-submit" not in backend_paths
