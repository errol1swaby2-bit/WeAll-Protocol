...
def _env(
    ...
    *,
    ...
    return TxEnvelope(
        ...
        sig="sig",
        system=system,
        parent="parent" if system else None,
    )
    ...
    return {
        ...
        "accounts": {
            "@author": {
                "nonce": 0,
                "poh_tier": 2,
                "session_keys": {"sk:@author": {"active": True}},
            },
            "@reviewer": {
                "nonce": 0,
                "poh_tier": 2,
                "session_keys": {"sk:@reviewer": {"active": True}},
            },
            "@member": {
                "nonce": 0,
                "poh_tier": 2,
                "session_keys": {"sk:@member": {"active": True}},
            },
        },
        "groups_by_id": {
            "g1": {
                "id": "g1",
                "visibility": "public",
                "members": {"@member": {"role": "member"}, "@author": {"role": "member"}},
            }
        },
        "roles": {
            "groups_by_id": {
...
def test_content_creator_can_appeal_but_reviewer_cannot() -> None:
                    ...
                    "actions": [
                        {
                            ...
                            "payload": {"target_id": "post:2", "visibility": "deleted"},
                        }
                    ]
                },
                "_due_height": 11,
    ...
    assert (
        st["disputes_by_id"]["d1"]["resolution"]["actions"][0]["payload"]["visibility"] == "hidden"
    )
    assert (
        st["disputes_by_id"]["d1"]["resolution"]["appeal_quarantine"]["content_record_retained"]
        is True
    )
    ...
    with pytest.raises(ApplyError) as exc:
        apply_tx(
            ...
            _env(
                "DISPUTE_APPEAL",
                {"dispute_id": "d1", "reason": "I reviewed this"},
                signer="@reviewer",
                nonce=3,
            ),
        )
    ...
    apply_tx(
        ...
        _env(
            "DISPUTE_APPEAL",
            {"dispute_id": "d1", "reason": "I created this content"},
            signer="@author",
            nonce=4,
        ),
    )
...
def test_dispute_detail_api_exposes_creator_only_appeal_eligibility() -> None:
    ...
    with _client(st) as client:
        author = client.get("/v1/disputes/d1", headers=_auth("@author")).json()["dispute"][
            "appeal_eligibility"
        ]
        reviewer = client.get("/v1/disputes/d1", headers=_auth("@reviewer")).json()["dispute"][
            "appeal_eligibility"
        ]
