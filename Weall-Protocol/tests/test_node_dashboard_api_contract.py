...
def test_frontend_contract_harness_starts_backend_before_contract_check() -> None:
    ...
    assert '"${PYTHON_BIN}" -m weall.api' in src
    assert '"${PYTHON_BIN}" - "${API_BASE}" "${LOG_FILE}"' in src
...
def test_web_ci_contract_backend_boots_real_runtime() -> None:
    workflow = Path("../.github/workflows/web-ci.yml").read_text()

    assert "python -m uvicorn weall.api.app:app" not in workflow
    assert "python -m weall.api" in workflow
    assert "WEALL_API_BOOT_RUNTIME=1" in workflow
    assert "WEALL_MODE=dev" in workflow
    assert "WEALL_DB_PATH=" in workflow
    assert "WEALL_TX_INDEX_PATH=" in workflow
