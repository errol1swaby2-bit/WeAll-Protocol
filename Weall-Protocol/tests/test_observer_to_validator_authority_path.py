...
def _live_tier2(st: dict) -> None:
    ...
    apply_tx(
        st,
        _env("POH_LIVE_JUROR_ACCEPT", {"case_id": case_id, "ts_ms": 2}, signer="live-j1", nonce=1),
    )
    apply_tx(
        st,
        _env(
...
def test_observer_account_progresses_to_validator_eligible_but_not_consensus_active_until_validator_set_update() -> (
    None
):
    st = _state()
    ...
    assert (
        st["accounts"]["@observer"]["devices"]["by_id"]["node:@observer"]["pubkey"] == node_pubkey
    )
    assert (
        apply_tx(
            st,
            _env(
                "PEER_ADVERTISE",
                {
                    "peer_id": "node:@observer",
                    "device_id": "node:@observer",
                    "node_pubkey": node_pubkey,
                    "endpoint": "https://observer.example",
                },
                nonce=3,
            ),
        )["applied"]
    ...
    assert (
        apply_tx(
            ...
            _env(
                ...
                {"peer_id": "genesis", "endpoint": "https://genesis.example"},
                nonce=4,
            ),
        )["applied"]
        == "PEER_REQUEST_CONNECT"
    ...
    assert (
        apply_tx(st, _env("ROLE_NODE_OPERATOR_ENROLL", {"account_id": "@observer"}, nonce=9))[
            "applied"
        ]
    ...
    assert (
        apply_tx(
            ...
            _env(
                ...
                nonce=6,
                system=True,
                parent="ROLE_NODE_OPERATOR_ENROLL",
            ),
        )["applied"]
        == "ROLE_NODE_OPERATOR_ACTIVATE"
    )
    ...
    assert (
        apply_tx(
            ...
            _env(
                ...
                nonce=7,
                system=True,
                parent="NODE_OPERATOR_VALIDATOR_OPT_IN",
            ),
        )["verified"]
        is True
    )
        ...
        apply_tx(
            ...
            _env(
                "VALIDATOR_CANDIDATE_REGISTER",
                {
                    "node_id": node_pubkey,
                    "pubkey": bft_pubkey,
                    "endpoints": ["https://observer.example"],
                },
                nonce=11,
            ),
        )["applied"]
    ...
    approved = apply_tx(
        ...
        _env(
            ...
            {"account": "@observer", "activate_at_epoch": 1},
            signer="SYSTEM",
            nonce=9,
            system=True,
            parent="gov:approve-observer",
        ),
    )
