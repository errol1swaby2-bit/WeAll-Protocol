...
def test_bft_timeout_check_emits_valid_genesis_timeout_for_non_leader(tmp_path: Path, monkeypatch) -> None:
    root = _repo_root()
...
def test_bft_timeout_check_skips_current_leader(tmp_path: Path, monkeypatch) -> None:
    ...
    ex = WeAllExecutor(db_path=str(tmp_path / "leader.db"), node_id="@v1", chain_id="bft-live", tx_index_path=tx_index_path)
    _seed_validator_set(ex, validators=validators, pub=vpub)
...
def test_bft_timeout_recovery_persists_view_and_ignores_stale_messages(tmp_path: Path) -> None:
    ...
    def timeout_from(signer: str, view: int) -> dict:
        ex2 = WeAllExecutor(db_path=str(tmp_path / f"sign-{signer}-{view}.db"), node_id=f"@{signer}", chain_id="bft-live", tx_index_path=tx_index_path)
    ...
    ex_restarted = WeAllExecutor(db_path=db_path, node_id="@v4", chain_id="bft-live", tx_index_path=tx_index_path)
