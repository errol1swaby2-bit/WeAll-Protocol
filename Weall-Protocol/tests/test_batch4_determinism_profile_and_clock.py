...
def test_restart_rejects_reputation_scale_mismatch(
...
def test_prod_restart_warns_but_allows_modest_tip_ahead_of_local_clock(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setenv("WEALL_MODE", "testnet")
    ...
    st2 = ex2.read_state()
    clock_warning = ((st2.get("meta") or {}) if isinstance(st2.get("meta"), dict) else {}).get(
        "clock_warning"
    )
    ...
    assert (
        int(clock_warning.get("startup_hard_fail_threshold_ms") or 0) == STARTUP_CLOCK_HARD_FAIL_MS
    )
    assert bool(clock_warning.get("startup_blocked", False)) is False
    assert ex2.observer_mode() is False


def test_prod_restart_warns_and_forces_observer_mode_when_tip_is_far_ahead_of_local_clock(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setenv("WEALL_MODE", "testnet")
    ...
    st2 = ex2.read_state()
    warning = ((st2.get("meta") or {}) if isinstance(st2.get("meta"), dict) else {}).get(
        "clock_warning"
    )
