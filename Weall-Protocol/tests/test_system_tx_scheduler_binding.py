...
def _live_state() -> dict:
    return {
        ...
        "accounts": {
            ...
            **{
                f"@j{i}": {"poh_tier": 2, "reputation_milli": 5000, "nonce": 1}
                for i in range(1, 12)
            },
        },
...
def test_system_queue_binding_rejects_missing_queue_id() -> None:
    ...
    ok, why = validate_system_tx_queue_binding(state, canon, forged, next_height=23, phase="post")
...
def test_system_queue_lookup_rejects_conflicting_duplicate_queue_id() -> None:
    state = _live_state()
    assert schedule_poh_live_system_txs(state, next_height=23) == 1
