...
def _env(
    tx_type: str, nonce: int, payload: dict[str, Any], *, parent: str | None = None
) -> TxEnvelope:
    if parent is None:
        parent = (
...
def _session_state() -> dict[str, Any]:
    return {
        ...
        "accounts": {
            "@alice": {
                "session_keys": {
                    "alice-session": {"active": True, "issued_at_ts": 1_800_000_000, "ttl_s": 3600}
                }
            },
            "@juror": {
                "session_keys": {
                    "juror-session": {"active": True, "issued_at_ts": 1_800_000_000, "ttl_s": 3600}
                }
            },
            "@mallory": {
                "session_keys": {
        ...
        "poh": {
            "async_cases": {
                "case-1": {
                    ...
                    "reviewer_restricted_evidence": {
                        "ev1": {"encrypted_blob_cid": "restricted-cid-do-not-leak"}
                    },
                    "evidence_binds": {
                        "bind:case-1:ev1": {"evidence_id": "ev1", "target_id": "case-1"}
                    },
            ...
            "live_sessions": {
                "session-1": {"case_id": "live-1", "status": "open", "room_commitment": "r" * 64}
            },
            "live_session_participants": {
                "session-1": {"@alice": {"status": "assigned"}, "@juror": {"status": "assigned"}}
            },
        },
    }
...
def test_sensitive_routes_have_explicit_non_generic_metadata() -> None:
    ...
    for route in sensitive:
        ...
        if any(
            marker in route["path"]
            for marker in (
                "/my-cases",
                "/juror-cases",
                "/assigned",
                "/presence",
                "/webrtc/signals/diagnostics",
                "/tx_queue/drain",
            )
        ):
...
def test_failure_registry_and_artifact_gate_artifacts_are_present() -> None:
    ...
    registry = json.loads(
        (ROOT / "generated/failure_code_registry_v1_5.json").read_text(encoding="utf-8")
    )
    ...
    gate_text = (ROOT / "scripts/check_v15_public_readiness_artifacts.py").read_text(
        encoding="utf-8"
    )
    for rel in [
        "generated/state_root_vectors_v1_5.json",
...
def test_poh_sensitive_case_read_requires_session_and_redacts_unrelated_viewer(monkeypatch) -> None:
    ...
    assert (
        juror.json()["case"]["reviewer_restricted_evidence"]["ev1"]["encrypted_blob_cid"]
        == "restricted-cid-do-not-leak"
    )
...
def test_poh_sensitive_case_read_withholds_raw_evidence_until_reviewer_accepts(monkeypatch) -> None:
    ...
    assert (
        accepted.json()["case"]["reviewer_restricted_evidence"]["ev1"]["encrypted_blob_cid"]
        == "restricted-cid-do-not-leak"
    )
...
def test_protocol_upgrade_activation_record_is_not_software_apply() -> None:
    ...
    apply_protocol(
        ...
        _env(
            ...
            1,
            {
                "upgrade_id": "upgrade-read-model",
                "version": "v1.5.1",
                "artifact_url": "https://example.invalid/no-fetch.tar.gz",
            },
        ),
    )
    out = apply_protocol(
    ...
    assert set(boundary["requested_execution_fields_ignored"]) == {
        "execute_migration",
        "rollback",
        "restart_node",
    }
...
def test_15_generated_artifacts_preserve_truth_boundaries() -> None:
    state_vectors = json.loads(
        (ROOT / "generated/state_root_vectors_v1_5.json").read_text(encoding="utf-8")
    )
    assert state_vectors["schema"] == "weall.v1_5.state_root_vectors"
    names = {v["name"] for v in state_vectors["vectors"]}
    assert {
        "poh_async_and_live_commitments",
    ...
    bft = json.loads(
        (ROOT / "generated/public_validator_bft_preflight_matrix_v1_5.json").read_text(
            encoding="utf-8"
        )
    )
