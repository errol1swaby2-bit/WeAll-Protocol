...
def test_seeded_demo_quickstart_enables_backend_secret_route() -> None:
...
def test_compose_passes_seeded_demo_secret_route_to_api_container() -> None:
    ...
    assert (
        "WEALL_ENABLE_DEV_BOOTSTRAP_SECRET_ROUTE: ${WEALL_ENABLE_DEV_BOOTSTRAP_SECRET_ROUTE:-1}"
        in text
    )
...
def test_full_stack_script_explicitly_enables_secret_route_for_browser_handoff() -> None:
    ...
    assert (
        'WEALL_ENABLE_DEV_BOOTSTRAP_SECRET_ROUTE="${WEALL_ENABLE_DEV_BOOTSTRAP_SECRET_ROUTE:-0}"'
        not in text
    )
