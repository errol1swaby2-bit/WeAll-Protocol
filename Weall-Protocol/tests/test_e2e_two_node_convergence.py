...
def test_two_node_convergence_smoke(tmp_path: Path) -> None:
    ...
    ex_a = WeAllExecutor(
        db_path=db_a, node_id="@alice", chain_id="conv", tx_index_path=tx_index_path
    )
...
def test_executor_refuses_chain_id_mismatch(tmp_path: Path) -> None:
    ...
    ex = WeAllExecutor(
        db_path=db_path, node_id="@alice", chain_id="chain-A", tx_index_path=tx_index_path
    )
    assert (
        ex.submit_tx(
            {
    ...
    with pytest.raises(ExecutorError):
        WeAllExecutor(
            db_path=db_path, node_id="@alice", chain_id="chain-B", tx_index_path=tx_index_path
        )
