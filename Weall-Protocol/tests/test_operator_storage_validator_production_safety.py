    ...
    issued = apply_tx(
        ...
        _env(
            ...
            2,
            {
                ...
                "challenge_id": "probe-1",
                "account_id": "@op",
                "node_pubkey": "node-pub-1",
                "reserved_capacity_bytes": 900_000,
                "sample_count": 3,
    ...
    with pytest.raises(ApplyError):
        apply_tx(
            ...
            _env(
                ...
                {
                    "challenge_id": "probe-1",
                    "response_commitment": "sha256:root",
                    "probe_responses": [
                        {"offset": offsets[0], "size": 1024, "response_hash": "sha256:one"}
                    ],
                },
            ),
        )
    ...
    apply_tx(
        ...
        _env(
            ...
            4,
            {
                ...
                "response_commitment": "sha256:root",
                "probe_responses": [
                    {"offset": o, "size": 1024, "response_hash": f"sha256:{i}"}
    ...
    verified = apply_tx(
        ...
        _env(
            ...
            5,
            {
                ...
                "verified_capacity_bytes": 800_000,
                "verification_receipt_hash": "sha256:receipt",
                "verifier_id": "SYSTEM",
            },
            system=True,
        ),
...
def test_legacy_storage_offer_without_responsibility_is_removed() -> None:
    st = {
        "height": 1,
        "accounts": {"@legacy": {"poh_tier": 2}},
        "storage": {"operators": {"@legacy": {"enabled": True, "capacity_bytes": 1_000_000}}},
    }
    with pytest.raises(ApplyError):
        apply_tx(
            st,
            _env(
                ...
                "@legacy",
                1,
                {"offer_id": "legacy", "capacity_bytes": 1_000},
            ),
        )
    pin = apply_tx(
        st,
        _env(
            "IPFS_PIN_REQUEST", "@user", 2, {"pin_id": "pin", "cid": VALID_CID, "size_bytes": 128}
        ),
    )
    assert pin["targets"] == []
...
def test_storage_proof_expiry_and_allocation_accounting_block_overallocation() -> None:
    ...
    issued = apply_tx(
        ...
        _env(
            ...
            {
                "proof_scope": "capacity_probe",
                "challenge_id": "probe-2",
                "account_id": "@op",
                "reserved_capacity_bytes": 100_000,
                "sample_count": 1,
                "sample_size_bytes": 512,
                "expires_height": 20,
            },
            system=True,
        ),
    ...
    apply_tx(
        ...
        _env(
            ...
            3,
            {
                ...
                "probe_responses": [{"offset": offset, "size": 512, "response_hash": "sha256:r0"}],
            },
        ),
    )
    apply_tx(
    ...
    apply_tx(
        st, _env("STORAGE_OFFER_CREATE", "@op", 5, {"offer_id": "offer", "capacity_bytes": 100_000})
    )
    apply_tx(
        st,
        _env(
            ...
            "@user",
            6,
            {"offer_id": "offer", "lease_id": "lease", "size_bytes": 60_000, "duration_blocks": 10},
        ),
    )
...
def test_validator_readiness_verification_required_before_active_validator_responsibility() -> None:
    ...
    apply_tx(
        ...
        _env(
            ...
            "@op",
            1,
            {"account_id": "@op", "node_pubkey": "node-pub-1"},
        ),
    )
