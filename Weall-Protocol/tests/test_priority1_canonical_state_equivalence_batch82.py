...
def _canonical_state(ex: WeAllExecutor) -> dict:
    st = ex.read_state()
    return {
...
def _canonical_hash(ex: WeAllExecutor) -> str:
    payload = json.dumps(
        _canonical_state(ex),
        sort_keys=True,
        separators=(",", ":"),
...
def test_restart_only_metadata_drift_does_not_change_canonical_state_batch82() -> None:
    ...
    try:
        ...
        ex = WeAllExecutor(db_path=db, node_id="n1", chain_id="b82", tx_index_path=tx_index_path)
        for signer in ["@a", "@b"]:
            assert _submit(ex, signer, 1).get("ok") is True
        while ex.read_mempool():
            assert ex.produce_block(max_txs=10).ok is True

        h1 = _canonical_hash(ex)

        ex2 = WeAllExecutor(db_path=db, node_id="n1", chain_id="b82", tx_index_path=tx_index_path)
        h2 = _canonical_hash(ex2)

        assert h1 == h2
        assert _canonical_state(ex) == _canonical_state(ex2)
    finally:
        Path(db).unlink(missing_ok=True)
...
def test_same_logical_work_different_restart_schedule_same_canonical_state_batch82() -> None:
    ...
    try:
        ...
        ex1 = WeAllExecutor(db_path=db1, node_id="n1", chain_id="b82x", tx_index_path=tx_index_path)
        ex2 = WeAllExecutor(db_path=db2, node_id="n1", chain_id="b82x", tx_index_path=tx_index_path)

        for signer in ["@c", "@d", "@e", "@f"]:
...
def test_rejected_duplicate_work_does_not_change_canonical_state_batch82() -> None:
    ...
    try:
        ...
        ex = WeAllExecutor(db_path=db, node_id="n1", chain_id="b82r", tx_index_path=tx_index_path)
        assert _submit(ex, "@x", 1).get("ok") is True
        while ex.read_mempool():
            assert ex.produce_block(max_txs=10).ok is True

        state1 = _canonical_state(ex)
        hash1 = _canonical_hash(ex)

        res = _submit(ex, "@x", 1)
        assert res.get("ok") is False

        state2 = _canonical_state(ex)
        hash2 = _canonical_hash(ex)
...
def test_cross_db_same_logical_work_same_canonical_hash_batch82() -> None:
    ...
    try:
        ...
        ex1 = WeAllExecutor(db_path=db1, node_id="n1", chain_id="b82z", tx_index_path=tx_index_path)
        ex2 = WeAllExecutor(db_path=db2, node_id="n2", chain_id="b82z", tx_index_path=tx_index_path)
