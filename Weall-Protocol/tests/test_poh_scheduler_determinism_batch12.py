...
def test_live_scheduler_is_env_independent(monkeypatch) -> None:
    monkeypatch.setenv("WEALL_POH_LIVE_MIN_REP", "9.999")

    state = _live_state()
    enq = schedule_poh_live_system_txs(state, next_height=23)

    assert enq == 1
    queue = state.get("system_queue") or []
    assert len(queue) == 1
    assert queue[0].get("tx_type") == "POH_LIVE_JUROR_ASSIGN"
    payload = dict(queue[0].get("payload") or {})
...
def test_legacy_float_thresholds_are_converted_once_to_integer_units() -> None:
    ...
    expected_live = sorted([f"@j{i}" for i in range(1, 12)])
    assert eligible_live_jurors(state=live_state, min_rep_units=5000) == expected_live
    assert eligible_live_jurors(state=live_state, min_rep="5.0") == expected_live
...
def test_scheduler_uses_integer_rep_threshold_payloads_for_legacy_thresholds() -> None:
    ...
    state3 = _live_state()
    state3["params"]["poh"].pop("live_min_rep_milli", None)
    state3["params"]["poh"]["live_min_rep"] = "5.0"

    enq3 = schedule_poh_live_system_txs(state3, next_height=23)
