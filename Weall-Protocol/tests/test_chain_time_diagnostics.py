...
def _make_executor(
    tmp_path: Path, *, node_id: str = "@n1", chain_id: str = "chain-time"
) -> WeAllExecutor:
...
def test_bft_diagnostics_expose_chain_time_floor_and_median(monkeypatch, tmp_path: Path) -> None:
    ...
    blk1, st1, applied1, invalid1, err1 = ex.build_block_candidate(
        allow_empty=True, force_ts_ms=1_000
    )
    assert err1 == ""
    meta1 = ex.commit_block_candidate(
        block=blk1, new_state=st1, applied_ids=applied1, invalid_ids=invalid1
    )
    assert meta1.ok is True

    blk2, st2, applied2, invalid2, err2 = ex.build_block_candidate(
        allow_empty=True, force_ts_ms=2_000
    )
    assert err2 == ""
    meta2 = ex.commit_block_candidate(
        block=blk2, new_state=st2, applied_ids=applied2, invalid_ids=invalid2
    )
    assert meta2.ok is True

    blk3, st3, applied3, invalid3, err3 = ex.build_block_candidate(
        allow_empty=True, force_ts_ms=4_000
    )
    assert err3 == ""
    meta3 = ex.commit_block_candidate(
        block=blk3, new_state=st3, applied_ids=applied3, invalid_ids=invalid3
    )
