...
def test_controlled_devnet_boot_uses_manifest_clock_and_empty_blocks() -> None:
    ...
    for src in (boot, join):
        ...
        assert (
            'WEALL_POH_ASYNC_APPROVAL_THRESHOLD="${WEALL_POH_ASYNC_APPROVAL_THRESHOLD:-1}"' in src
        )
    ...
    expected_tx_index_hash = hashlib.sha256(
...
def test_native_async_uses_genesis_single_reviewer_policy() -> None:
    ...
    assert (
        'ASYNC_JUROR_COUNT="${WEALL_ASYNC_JUROR_COUNT:-${WEALL_POH_ASYNC_N_JURORS:-1}}"' in native
    )
