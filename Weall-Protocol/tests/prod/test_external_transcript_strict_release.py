...
def _with_digest(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    payload["transcript_digest"] = hashlib.sha256(
...
def _run_validator(
    kind: str, path: Path, *, strict: bool = False
...
def test_strict_release_rejects_validator_and_storage_scaffolds(tmp_path: Path) -> None:
    ...
    from rehearse_external_public_validator_operator_network_v1_5 import (
        build_transcript as build_validator,
    )
    from rehearse_storage_ipfs_external_operator_topology_v1_5 import (
        build_transcript as build_storage,
    )
...
def test_strict_release_accepts_attested_validator_transcript(tmp_path: Path) -> None:
    state_roots = {
        node: "f" * 64
        for node in ("validator-alpha", "validator-beta", "validator-gamma", "validator-delta")
    }
    payload = _with_digest(
        {
            ...
            "machine_ids": [
                "host-audit-alpha-001",
                "host-audit-beta-002",
                "host-audit-gamma-003",
                "host-audit-delta-004",
            ],
            "rounds": 8,
            "threshold": 3,
            "fresh_clone": True,
            ...
            "restart_replay": True,
            "operator_signatures": [
                "sig-mldsa-alpha-real-attestation-0001",
                "sig-mldsa-beta-real-attestation-0002",
                "sig-mldsa-gamma-real-attestation-0003",
                "sig-mldsa-delta-real-attestation-0004",
            ],
    ...
    storage = _with_digest(
        {
            ...
            "machine_ids": [
                "storage-host-alpha-001",
                "storage-host-beta-002",
                "storage-host-gamma-003",
            ],
            "ipfs_peer_ids": [
                "12D3KooWalphaRealPeer111111",
                "12D3KooWbetaRealPeer222222",
                "12D3KooWgammaRealPeer333333",
            ],
            "daemon_versions": {
    ...
    legal = _with_digest(
        {
            ...
            "review_commit": "abc123def4567890abc123def4567890abc123de",
...
def test_release_evidence_manifest_is_fresh_and_conservative() -> None:
    ...
    assert (
        payload["release_evidence_gates"]["external_validator_operator_transcript"][
            "sample_transcripts_are_rejected_in_strict_release"
        ]
        is True
    )
