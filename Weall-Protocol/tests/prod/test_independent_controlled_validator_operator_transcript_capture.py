...
def test_independent_validator_capture_script_is_helpful_and_non_authoritative(
    tmp_path: Path,
) -> None:
    script = (
        ROOT / "scripts" / "capture_independent_controlled_validator_operator_transcript_v1_5.sh"
    )
    ...
    assert (
        "Captures one machine's independent controlled validator/operator rehearsal evidence packet"
        in help_proc.stdout
    )
    ...
    packet = json.loads(
        (out_dir / "LOCAL_VALIDATOR_OPERATOR_EVIDENCE.json").read_text(encoding="utf-8")
    )
...
def test_independent_validator_schema_and_validator_accept_real_shape(tmp_path: Path) -> None:
    ...
    transcript_path.write_text(
        json.dumps(_sample_validator_transcript(), indent=2, sort_keys=True), encoding="utf-8"
    )
    validate = _run(
        sys.executable,
...
def test_public_beta_and_release_artifacts_reference_independent_validator_capture() -> None:
    ...
    assert (
        "capture_independent_controlled_validator_operator_transcript_v1_5.sh"
        in gate["capture_script"]
    )
