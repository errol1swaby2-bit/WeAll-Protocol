...
def test_production_helper_topology_hardening_plan_artifact_is_fresh_and_open() -> None:
    proc = subprocess.run(
        [
            sys.executable,
            "scripts/gen_production_helper_topology_hardening_plan_v1_5.py",
            "--check",
        ],
        cwd=str(ROOT),
        text=True,
    ...
    payload = json.loads(
        (ROOT / "generated" / "production_helper_topology_hardening_plan_v1_5.json").read_text(
            encoding="utf-8"
        )
    )
...
def test_production_helper_topology_docs_and_template_preserve_non_claims() -> None:
    docs = [
        ...
        / "docs"
        / "proofs"
        / "production-helper-topology-hardening"
        / "2026-07-05"
        / "README.md",
    ]
    for path in docs:
    ...
    assert (
        template["current_disabled_boundary"]["launch_matrix_blocks_production_helper_execution"]
        is True
    )
