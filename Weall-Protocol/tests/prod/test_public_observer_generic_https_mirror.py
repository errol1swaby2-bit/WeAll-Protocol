...
def test_generic_https_mirror_is_accepted_without_provider_specific_logic(
    ...
    monkeypatch.setenv(
        ...
        _trust_roots(
            ...
            [
                {
                    "url": "https://mirror.example.org/public_testnet_seed_registry.json",
                    "provider": "anything",
                }
            ],
        ),
    ...
    monkeypatch.setattr(
        public_seed_registry, "_http_get_registry_json", lambda _url, *, timeout_s: remote
    )
...
def test_bad_generic_mirror_falls_back_to_checked_in_registry(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setenv(
        "WEALL_PUBLIC_TESTNET_TRUST_ROOTS_PATH",
        _trust_roots(tmp_path, ["https://mirror.example.org/bad.json"]),
    )
    registry_dir = tmp_path / "Weall-Protocol" / "configs"
    registry_dir.mkdir(parents=True)
    fallback = registry_dir / "public_testnet_seed_registry.json"
    fallback.write_text(
        json.dumps(signed_registry(_registry(seed_p2p_urls=["tls://fallback.example.org:30303"]))),
        encoding="utf-8",
    )
...
def test_wrong_signature_is_rejected_even_if_mirror_metadata_claims_authority(
    tmp_path, monkeypatch
) -> None:
    _public_env(monkeypatch)
    monkeypatch.setenv(
        ...
        _trust_roots(
            ...
            [
                {
                    "url": "https://mirror.example.org/registry.json",
                    "provider": "trusted",
                    "authority": True,
                }
            ],
    ...
    monkeypatch.setattr(
        public_seed_registry, "_http_get_registry_json", lambda _url, *, timeout_s: bad
    )
