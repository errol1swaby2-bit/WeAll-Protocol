...
def test_remote_signed_seed_registry_url_is_a_valid_hybrid_source(monkeypatch):
    ...
    monkeypatch.setenv(
        ...
        "http://127.0.0.1:9999/public_testnet_seed_registry.json",
    )
...
def test_hybrid_registry_falls_back_to_checked_in_default_when_remote_unreachable(
    tmp_path, monkeypatch
):
    _public_env(monkeypatch)
    monkeypatch.setenv(
        "WEALL_PUBLIC_TESTNET_SEED_REGISTRY_URL", "http://127.0.0.1:9999/missing.json"
    )
    registry_dir = tmp_path / "Weall-Protocol" / "configs"
    registry_dir.mkdir(parents=True)
    registry_path = registry_dir / "public_testnet_seed_registry.json"
    registry_path.write_text(
        json.dumps(signed_registry(_registry(chain_id="weall-testnet-fallback"))), encoding="utf-8"
    )
    monkeypatch.chdir(tmp_path)

    def fake_fetch(_url: str, *, timeout_s: float):
        raise public_seed_registry.PublicSeedRegistryError(
            "public_seed_registry_remote_fetch_failed"
        )
...
def test_public_mode_peer_addr_gossip_requires_signed_records(monkeypatch):
    ...
    node._handle_peer_addr(
        "peer-x", PeerAddrMsg(header=_header(MsgType.PEER_ADDR), addrs=(unsigned, signed))
    )
