...
def _seed_bft_executor(
    db_path: str, chain_id: str, validators: list[str], vpub: dict[str, str]
) -> WeAllExecutor:
    ex = WeAllExecutor(
        db_path=db_path,
...
def test_block_commit_failpoint_before_ledger_state_preserves_atomicity(
...
def test_bft_finalized_frontier_failpoint_does_not_partially_persist(
    tmp_path: Path, monkeypatch
) -> None:
    ...
    try:
        ex.bft_handle_qc(qc)
        raise AssertionError("expected failpoint-triggered exception")
