...
def test_multiple_signers_can_apply_in_one_chain(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    for i in range(3):
        assert ex.submit_tx(
            {"tx_type": "ACCOUNT_REGISTER", "signer": f"@user{i:03d}", "nonce": 1, "payload": {"pubkey": f"k:u{i:03d}"}}
        )["ok"] is True
