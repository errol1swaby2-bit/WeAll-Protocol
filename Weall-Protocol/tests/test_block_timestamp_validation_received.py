...
def test_apply_block_rejects_timestamp_beyond_chain_time_window(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    sub = leader.submit_tx({"tx_type": "ACCOUNT_REGISTER", "signer": "@user000", "nonce": 1, "payload": {"pubkey": "k:0"}})
    assert sub["ok"] is True

    future_ts_ms = MAX_BLOCK_TIME_ADVANCE_MS + 60_000
    blk, _st2, _applied, _invalid, err = leader.build_block_candidate(max_txs=1, force_ts_ms=future_ts_ms)
...
def test_apply_block_rejects_timestamp_before_tip(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("WEALL_UNSAFE_DEV", "1")
    monkeypatch.setenv("WEALL_SIGVERIFY", "0")
    ...
    blk1, _st1, _applied1, _invalid1, err1 = leader.build_block_candidate(max_txs=1, force_ts_ms=1_000)
    assert err1 == ""
...
def test_build_block_candidate_rejects_timestamp_before_tip(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("WEALL_UNSAFE_DEV", "1")
    monkeypatch.setenv("WEALL_SIGVERIFY", "0")
    ...
    blk2, _st2, _applied2, _invalid2, err2 = ex.build_block_candidate(max_txs=1, allow_empty=True, force_ts_ms=tip_ts_ms - 1)
    assert blk2 is None
    assert err2 == "invalid_block_ts:before_chain_floor"


def test_build_block_candidate_accepts_timestamp_independent_of_wall_clock(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("WEALL_UNSAFE_DEV", "1")
    monkeypatch.setenv("WEALL_SIGVERIFY", "0")

    ex = _make_executor(tmp_path, node_id="@leader", chain_id="ts-local-2")
    blk, _st2, _applied2, _invalid2, err = ex.build_block_candidate(allow_empty=True, force_ts_ms=MAX_BLOCK_TIME_ADVANCE_MS)
    assert err == ""
