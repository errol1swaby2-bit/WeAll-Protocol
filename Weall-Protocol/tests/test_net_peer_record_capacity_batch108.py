...
def _hello_pkt(peer_id: str, *, received_at_ms: int) -> WirePacket:
    ...
    return WirePacket(peer_id=peer_id, payload=encode_message(hello), received_at_ms=received_at_ms, meta=None)
...
def test_peer_record_capacity_evicts_oldest_unestablished_peer_batch108() -> None:
    ...
    node._handle_packet(WirePacket(peer_id="peer-a", payload=b"not-json", received_at_ms=1, meta=None))
...
def test_peer_record_capacity_preserves_established_session_batch108() -> None:
    ...
    node._handle_packet(WirePacket(peer_id="peer-transient", payload=b"bad", received_at_ms=11, meta=None))
