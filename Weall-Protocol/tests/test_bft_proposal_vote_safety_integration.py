...
def test_bft_on_proposal_refuses_same_view_conflicting_vote(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    ex.state.setdefault("consensus", {}).setdefault("validators", {}).setdefault("registry", {})["@val1"] = {"pubkey": "pub"}
    ex._bft.locked_qc = QuorumCert(chain_id="test-chain", view=1, block_id="lock", block_hash="lock-h", parent_id="genesis", votes=tuple())
    ...
    good = {
        ...
        "block": {
            "block_id": "good",
            "header": {"chain_id": "test-chain", "height": 1, "prev_block_hash": "00" * 32, "block_ts_ms": 1000, "tx_ids": [], "receipts_root": ""},
            "prev_block_id": "lock"
        },
    }
    bad = {
        ...
        "block": {
            "block_id": "bad",
            "header": {"chain_id": "test-chain", "height": 1, "prev_block_hash": "00" * 32, "block_ts_ms": 1001, "tx_ids": [], "receipts_root": ""},
...
def test_bft_on_proposal_refuses_vote_when_replay_validation_fails(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("WEALL_MODE", "testnet")
    monkeypatch.setenv("WEALL_AUTOVOTE", "1")
    monkeypatch.setenv("WEALL_BFT_ENABLED", "1")
    monkeypatch.setenv("WEALL_BFT_ALLOW_QC_LESS_BLOCKS", "1")
