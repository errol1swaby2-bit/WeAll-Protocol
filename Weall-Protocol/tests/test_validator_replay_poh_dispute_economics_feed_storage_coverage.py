...
def test_poh_upheld_challenge_flags_prior_approving_reviewers() -> None:
    ...
    apply_poh(
        ...
        _env(
            ...
            signer="bob",
            nonce=1,
            payload={"account_id": "alice", "case_id": "case-1", "reason": "duplicate"},
        ),
    )
    out = apply_poh(
        state,
...
def test_dispute_target_registry_and_unsupported_action_rejection() -> None:
    state = {
        "height": 1,
        "accounts": {"alice": {"poh_tier": 1, "poh_status": "active"}, "bob": {}},
        "disputes_by_id": {},
    }
        ...
        _env(
            ...
            payload={
                ...
                "appeal_resolution": {
                    "decision": "modify",
                    "actions": [
                        {"tx_type": "UNKNOWN_ACTION", "payload": {}},
                        {
...
def test_economics_activation_preconditions_are_enforceable_but_opt_in() -> None:
    state = {
        "height": 10,
        "time": 2_000_000_000,
        "params": {"economic_unlock_time": 1, "economics_enabled": False},
        "accounts": {"SYSTEM": {}},
    }
    with pytest.raises(EconomicsApplyError) as exc:
        apply_economics(
            ...
            _env(
                ...
                system=True,
                nonce=1,
                payload={"enable": True, "enforce_preconditions": True},
            ),
        )
    ...
    state["economics"] = {
        "reward_policy": {"eligible_roles": ["validator", "juror"]},
        "wallet_policy": {"initialization": "explicit"},
        "fee_policy": {"post_fee_int": 0},
    }
    out = apply_economics(
        state,
        _env(
...
def test_storage_pin_confirm_can_record_retrieval_availability() -> None:
    state = {
        ...
        "storage": {
            "operators": {"opA": {"enabled": True, "capacity_bytes": 1000}},
            "pins": {
                "pin-1": {
                    "pin_id": "pin-1",
                    "cid": CID_A,
...
def test_feed_ranking_is_deterministic_and_default_compatible() -> None:
    ...
    recency = _sort_feed_items(
        [dict(x, feed_rank_score=_feed_rank_score(x, mode="recency")) for x in items],
        mode="recency",
    )
