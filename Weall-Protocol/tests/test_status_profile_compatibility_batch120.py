...
def test_status_surfaces_profile_compatibility_and_config_summary(tmp_path: Path, monkeypatch) -> None:
    ...
    assert compat["peer_profile_enforcement"] == "strict"
    assert compat["config_source_summary"]["env_applied"] is True
    assert "WEALL_NODE_LIFECYCLE_STATE" in compat["config_source_summary"]["env_vars_applied"]

    nested = body["operator"]["profile_compatibility"]
    assert nested["profile_commitment"] == compat["profile_commitment"]

    consensus = client.get("/v1/status/consensus")
    assert consensus.status_code == 200
    consensus_body = consensus.json()
    assert consensus_body["profile_compatibility"]["authority_ready"] is True
    assert consensus_body["node_lifecycle"]["peer_profile_enforcement"] == "strict"
