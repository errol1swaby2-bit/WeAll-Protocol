...
def test_constitution_hash_is_bound_into_canonical_chain_manifest() -> None:
    manifest = load_chain_manifest(
        str(ROOT / "configs" / "chains" / "weall-genesis.json"), required=True
    )
    assert manifest is not None
    expected_hash = hashlib.sha256(
        (ROOT / "docs" / "constitution" / "WEALL_GENESIS_CONSTITUTION_DRAFT_2.md").read_bytes()
    ).hexdigest()
...
def test_chain_manifest_status_reports_constitution_commitments() -> None:
    ...
    report = chain_manifest_status(
        manifest=manifest, chain_id="weall-prod", mode="prod", strict=True
    )
