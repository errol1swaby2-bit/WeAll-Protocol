...
from weall.runtime.apply.protocol import (
    ProtocolApplyError,
    apply_protocol,
    constitution_effective_view,
)
...
def test_constitution_upgrade_rejects_invalid_hash_private_evidence_and_rights_floor_bypass() -> (
    None
):
    with pytest.raises(ProtocolApplyError) as bad_hash:
        apply_protocol(
            ...
            _env(
                "CONSTITUTION_UPGRADE_DECLARE",
                1,
                _declare_payload(constitution_id="bad", document_hash="not-a-sha"),
            ),
        )
    ...
    with pytest.raises(ProtocolApplyError) as private_evidence:
        apply_protocol(
            ...
            _env(
                "CONSTITUTION_UPGRADE_DECLARE",
                2,
                _declare_payload(constitution_id="private", government_id="raw-id"),
            ),
        )
    assert (
        private_evidence.value.reason == "constitution_upgrade_contains_private_identity_evidence"
    )
    ...
    with pytest.raises(ProtocolApplyError) as bypass:
        apply_protocol(
            ...
            _env(
                "CONSTITUTION_UPGRADE_DECLARE",
                3,
                _declare_payload(constitution_id="bypass", bypass_rights_floor=True),
            ),
        )
...
def test_constitution_activation_is_future_height_scheduled_and_record_only() -> None:
    ...
    with pytest.raises(ProtocolApplyError) as not_future:
        apply_protocol(
            ...
            _env(
                ...
                2,
                {"constitution_id": "const-v0-2", "activation_height": 21},
            ),
        )
    ...
    out = apply_protocol(
        ...
        _env(
            "CONSTITUTION_UPGRADE_ACTIVATE",
            3,
            {"constitution_id": "const-v0-2", "activation_height": 60},
        ),
    )
    assert out is not None
...
def test_constitution_activation_rejects_mismatched_version_or_hash_and_duplicate_conflict() -> (
    None
):
    state = {"height": 30}
    apply_protocol(state, _env("CONSTITUTION_UPGRADE_DECLARE", 1, _declare_payload()))
    ...
    with pytest.raises(ProtocolApplyError) as version_mismatch:
        apply_protocol(
            ...
            _env(
                "CONSTITUTION_UPGRADE_ACTIVATE",
                2,
                {"constitution_id": "const-v0-2", "version": "v0.3", "activation_height": 80},
            ),
        )
    ...
    first = apply_protocol(
        ...
        _env(
            "CONSTITUTION_UPGRADE_ACTIVATE",
            4,
            {"constitution_id": "const-v0-2", "activation_height": 80},
        ),
    )
    ...
    with pytest.raises(ProtocolApplyError) as duplicate_conflict:
        apply_protocol(
            ...
            _env(
                "CONSTITUTION_UPGRADE_ACTIVATE",
                6,
                {"constitution_id": "const-v0-2", "activation_height": 81},
            ),
        )
...
def test_constitution_effective_view_uses_block_height_without_mutating_state() -> None:
    state = {
        "height": 40,
        "constitution": {"active": {"version": "v0.1", "document_hash": "sha256:" + "0" * 64}},
    }
    apply_protocol(state, _env("CONSTITUTION_UPGRADE_DECLARE", 1, _declare_payload()))
    apply_protocol(
        state,
        _env(
            "CONSTITUTION_UPGRADE_ACTIVATE",
            2,
            {"constitution_id": "const-v0-2", "activation_height": 75},
        ),
    )
...
def test_constitution_upgrade_lifecycle_replays_to_identical_state_roots() -> None:
    txs = [
        ...
        _env(
            "CONSTITUTION_UPGRADE_ACTIVATE",
            2,
            {"constitution_id": "const-v0-2", "activation_delay_blocks": 12, "_due_height": 90},
        ),
    ]
