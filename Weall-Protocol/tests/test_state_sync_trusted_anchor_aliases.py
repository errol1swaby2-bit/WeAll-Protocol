...
def test_state_sync_accepts_legacy_and_new_trusted_anchor_env_alias(
    ...
    req = StateSyncRequestMsg(
        header=_hdr(), mode="snapshot", selector={"trusted_anchor": build_snapshot_anchor(st)}
    )
    resp = svc.handle_request(req)
    assert resp.ok is True


def test_state_sync_fails_closed_on_conflicting_trusted_anchor_aliases(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("WEALL_STATE_SYNC_REQUIRE_TRUSTED_ANCHOR", "1")
