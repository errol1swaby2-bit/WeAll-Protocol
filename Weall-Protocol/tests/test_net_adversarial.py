...
from weall.net.messages import MsgType, PeerHello, WireHeader, BlockVoteMsg
...
def test_protocol_mismatch_chain_id_fast_bans() -> None:
    ...
    hdr_bad = WireHeader(type=MsgType.PEER_HELLO, chain_id="WRONG", schema_version="1", tx_index_hash="deadbeef")
    hello_bad = PeerHello(header=hdr_bad, peer_id="tcp://9.9.9.9:7777", agent="t", nonce="n", caps=())
...
def test_protocol_mismatch_schema_version_fast_bans() -> None:
    ...
    hdr_bad = WireHeader(type=MsgType.PEER_HELLO, chain_id="test", schema_version="999", tx_index_hash="deadbeef")
    hello_bad = PeerHello(header=hdr_bad, peer_id="tcp://1.2.3.4:5555", agent="t", nonce="n", caps=())
...
def test_protocol_mismatch_tx_index_hash_fast_bans() -> None:
    ...
    hdr_bad = WireHeader(type=MsgType.PEER_HELLO, chain_id="test", schema_version="1", tx_index_hash="BADHASH")
    hello_bad = PeerHello(header=hdr_bad, peer_id="tcp://5.6.7.8:9999", agent="t", nonce="n", caps=())
    node._handle_packet(_pkt("tcp://5.6.7.8:9999", hello_bad))

    assert node.is_banned("tcp://5.6.7.8:9999")


def test_identity_required_without_ledger_provider_bans_fast(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    hello = PeerHello(header=hdr, peer_id="tcp://9.9.9.9:7777", agent="t", nonce="n", caps=(), identity={"x": 1})
