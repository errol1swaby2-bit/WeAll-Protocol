...
def test_restart_rejects_genesis_bootstrap_profile_drift(
    ...
    ex = WeAllExecutor(
        db_path=db_path, node_id=acct, chain_id="batch126-genesis", tx_index_path=_tx_index_path()
    )
    st = ex.read_state()
    meta = st.get("meta") if isinstance(st.get("meta"), dict) else {}
    assert meta.get("genesis_bootstrap_profile", {}).get("enabled") is True
    assert isinstance(meta.get("genesis_bootstrap_profile_hash"), str) and meta.get(
    ...
    with pytest.raises(ExecutorError, match="genesis_bootstrap_profile mismatch"):
        WeAllExecutor(
            db_path=db_path,
            node_id=acct,
            chain_id="batch126-genesis",
            tx_index_path=_tx_index_path(),
        )


def test_restart_rejects_late_genesis_bootstrap_enablement(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    acct = "@plain-genesis"
    ...
    ex = WeAllExecutor(
        db_path=db_path, node_id=acct, chain_id="batch126-plain", tx_index_path=_tx_index_path()
    )
...
def test_genesis_mode_profile_is_pinned_to_validator_identity(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    ex = WeAllExecutor(
        db_path=db_path, node_id=acct, chain_id="batch126-mode", tx_index_path=_tx_index_path()
    )
    meta = ex.read_state().get("meta") if isinstance(ex.read_state().get("meta"), dict) else {}
