...
from weall.runtime.block_admission import (
    _current_validator_set_hash_from_state,
    _validator_set_hash_from_validators,
)
...
def test_block_admission_fallback_set_hash_matches_canonical_hash_when_state_has_no_persisted_set_hash() -> (
    None
):
    state = {
    ...
    assert _current_validator_set_hash_from_state(state) == validator_set_hash(
        ["v1", "v2", "v3", "v4"]
    )
