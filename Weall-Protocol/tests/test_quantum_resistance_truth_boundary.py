...
DOCS = [
    ...
    ROOT / "SECURITY.md",
    ROOT / "docs" / "security" / "CRYPTO_AGILITY_AND_QUANTUM_POSTURE.md",
]


def test_docs_do_not_claim_quantum_proof_or_completed_crypto_audit():
    combined = "\n".join(
        p.read_text(encoding="utf-8", errors="replace") for p in DOCS if p.exists()
    ).lower()
...
def test_docs_preserve_pre_public_testnet_hardening_frame():
    text = (ROOT / "docs" / "security" / "CRYPTO_AGILITY_AND_QUANTUM_POSTURE.md").read_text(
        encoding="utf-8"
    )
