...
def test_pending_fetch_request_descriptors_include_expected_hash_for_missing_parent(tmp_path: Path, monkeypatch) -> None:
    ...
    qc2 = _make_qc(
        ...
        block_hash=str(block2.get("block_hash") or ""),
    ...
    descs = follower.bft_pending_fetch_request_descriptors()
    assert descs == [
        {
            "block_id": str(block1["block_id"]),
            "block_hash": str(block1.get("block_hash") or ""),
            "reason": "missing_parent",
            "child_block_id": str(block2["block_id"]),
        }
    ]
    diag = follower.bft_diagnostics()
    assert diag["pending_fetch_request_hashes"] == [str(block1.get("block_hash") or "")]
    assert diag["pending_fetch_request_hashes_count"] == 1


def test_net_loop_rejects_mismatched_fetched_block_hash_when_expected_known(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("WEALL_MODE", "testnet")
    monkeypatch.setenv("WEALL_BFT_ENABLED", "1")
    ...
    assert follower.bft_on_proposal({"view": 2, "proposer": "v2", "block": block2}) is None
    assert follower.bft_pending_fetch_request_descriptors()[0]["block_hash"] == str(block1.get("block_hash") or "")
    ...
    evil_block1 = json.loads(json.dumps(block1))
    evil_block1["block_hash"] = "evil-hash"

    def _fake_get(url: str, *, timeout_s: float = 2.0):
        assert url.endswith(f"/v1/state/block/{block1['block_id']}")
        return {"ok": True, "block": evil_block1}
