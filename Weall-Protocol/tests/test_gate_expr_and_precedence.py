...
def test_and_binds_tighter_than_or() -> None:
    ...
    ok, rej = admit_tx({"tx_type": "X", "signer": "@user", "nonce": 1, "payload": {}, "sig": "x"}, ledger, canon, "mempool")
    assert ok is False
    ...
    ok2, rej2 = admit_tx({"tx_type": "X", "signer": "@user", "nonce": 1, "payload": {}, "sig": "x"}, ledger2, canon, "mempool")
    assert ok2 is True
    assert rej2 is None
