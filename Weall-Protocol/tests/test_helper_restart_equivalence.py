...
def _materialize_lane_results(*, lane_plans, plan_id: str, shared_key: str | None = None):
    ...
    for idx, lane_plan in enumerate(helper_lanes, start=1):
        receipts = [
            {"tx_id": tx_id, "ok": True, "lane_id": lane_plan.lane_id} for tx_id in lane_plan.tx_ids
        ]
        delta_key = shared_key if shared_key else f"state/{lane_plan.lane_id.lower()}"
        state_delta = {
            delta_key
            if lane_plan in accepted_lanes
            else f"state/{lane_plan.lane_id.lower()}": f"value-{idx}"
        }
...
def test_helper_restart_snapshot_is_restart_equivalent_with_mixed_helper_and_fallback(
    tmp_path,
) -> None:
    lane_plans, plan_id = lane_setup(txs=MULTILANE_TXS)
    helper_lanes, _accepted_lanes, _fallback_lane, lane_results_by_id, helper_pubkeys, certs = (
        _materialize_lane_results(
    ...
    profile = HelperAssemblyProfile(
        helper_mode_enabled=True, require_serial_equivalence=True, fail_closed_on_helper_error=True
    )
    ...
    snapshot1 = build_helper_restart_snapshot(
        ...
        helper_pubkeys=helper_pubkeys,
        helper_timeout_ms=50,
        serial_equivalence_fn=lambda candidates: (
            tuple(item.lane_id for item in candidates) == expected_lanes
        ),
    )
    ...
    reversed_results = {
        lane_id: lane_results_by_id[lane_id]
        for lane_id in reversed(tuple(lane_results_by_id.keys()))
    }
...
def test_helper_restart_snapshot_keeps_fail_closed_vs_serial_fallback_stable_on_merge_conflict(
    tmp_path,
) -> None:
    lane_plans, plan_id = lane_setup(txs=MULTILANE_TXS)
    helper_lanes, _accepted_lanes, _fallback_lane, lane_results_by_id, helper_pubkeys, certs = (
    ...
    strict_profile = HelperAssemblyProfile(
        helper_mode_enabled=True, require_serial_equivalence=True, fail_closed_on_helper_error=True
    )
    ...
    permissive_profile = HelperAssemblyProfile(
        helper_mode_enabled=True, require_serial_equivalence=True, fail_closed_on_helper_error=False
    )
