...
def _make_executor() -> WeAllExecutor:
    ...
    ex._bft_pending_block_json = MethodType(
        lambda self, bid: (
            dict(self._pending_remote_blocks.get(str(bid), {}))
            if str(bid) in self._pending_remote_blocks
            else None
        ),
        ex,
    )
    ex._bft_block_is_applyable_finalized_descendant = MethodType(
        lambda self, blk, finalized: True, ex
    )
    ...
    ex._drop_pending_candidate_artifacts = MethodType(
        lambda self, bid: (
            self._drops.append(str(bid)),
            self._pending_remote_blocks.pop(str(bid), None),
        ),
        ex,
    )
    ex._applied = []
    ex.apply_block = MethodType(
        lambda self, blk: (
            self._applied.append(str(blk.get("block_id") or "")),
            ExecutorMeta(ok=True, block_id=str(blk.get("block_id") or "")),
        )[1],
        ex,
    )
    return ex
