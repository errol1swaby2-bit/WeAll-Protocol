...
def test_partition_heal_replays_missing_chain_without_conflicting_tip(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, heal_order: tuple[int, int, int]) -> None:
    ...
    follower_a = WeAllExecutor(db_path=str(tmp_path / "follower-a.db"), node_id="v3", chain_id="bft-live", tx_index_path=tx_index_path)
    follower_b = WeAllExecutor(db_path=str(tmp_path / "follower-b.db"), node_id="v4", chain_id="bft-live", tx_index_path=tx_index_path)
    ...
    for i in (1, 2, 3):
        assert follower_a.bft_on_proposal({"view": i, "proposer": leader_for_view(validators, i), "block": proposals[i]}) is None
    ...
    assert follower_b.bft_on_proposal({"view": 3, "proposer": leader_for_view(validators, 3), "block": proposals[3]}) is None
    ...
    for i in heal_order:
        assert follower_b.bft_on_proposal({"view": i, "proposer": leader_for_view(validators, i), "block": proposals[i]}) is None
...
def test_out_of_order_qc_buffering_and_stale_replay_do_not_duplicate_execution(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("WEALL_MODE", "testnet")
    monkeypatch.setenv("WEALL_BFT_ENABLED", "1")
    ...
    assert follower.bft_on_proposal({"view": 2, "proposer": leader_for_view(validators, 2), "block": block2j}) is None
    assert int(follower.state.get("height") or 0) == 0
    ...
    assert follower.bft_on_proposal({"view": 1, "proposer": leader_for_view(validators, 1), "block": block1j}) is None
...
def test_validator_set_transition_rejects_late_old_epoch_messages_after_activation(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("WEALL_MODE", "testnet")
    monkeypatch.setenv("WEALL_BFT_ENABLED", "1")
    ...
    follower = WeAllExecutor(db_path=str(tmp_path / "follower.db"), node_id="v5", chain_id="bft-live", tx_index_path=tx_index_path)
    ...
    assert follower.bft_on_proposal({"view": 7, "proposer": leader_for_view(old_validators, 7), "block": old_block}) is None
    assert int(follower.state.get("height") or 0) == 0
    assert follower.bft_pending_fetch_requests() == []

    leader_new = WeAllExecutor(db_path=str(tmp_path / "leader-new.db"), node_id="v2", chain_id="bft-live", tx_index_path=tx_index_path)
    ...
    assert follower.bft_on_proposal({"view": 8, "proposer": leader_for_view(new_validators, 8), "block": blkj}) is None
...
def test_crash_mid_commit_preserves_vote_safety_across_restart(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    tx_index_path = str(_repo_root() / "generated" / "tx_index.json")
    validators, vpub, vpriv = _mk_validators(["v1", "v2", "v3", "v4"])
    ...
    ex = WeAllExecutor(db_path=db_path, node_id="v2", chain_id="bft-live", tx_index_path=tx_index_path)
    _seed_validator_set(ex, validators=validators, pub=vpub, epoch=3)
    ...
    ex2 = WeAllExecutor(db_path=db_path, node_id="v2", chain_id="bft-live", tx_index_path=tx_index_path)
...
def test_epoch_transition_prunes_stale_pending_artifacts(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    follower = WeAllExecutor(db_path=str(tmp_path / "follower-prune.db"), node_id="v5", chain_id="bft-live", tx_index_path=tx_index_path)
    _seed_validator_set(follower, validators=new_validators, pub=new_pub, epoch=4)
