...
def _seed_hex(priv: Ed25519PrivateKey) -> str:
    return priv.private_bytes(encoding=Encoding.Raw, format=PrivateFormat.Raw, encryption_algorithm=NoEncryption()).hex()
...
def test_prod_requires_chain_id_bound_signature(monkeypatch) -> None:
    ...
    state = {"chain_id": "chain-A", "params": {"require_signatures": True}, "accounts": {"@alice": {"keys": [{"pubkey": pub, "active": True}]}}}
...
def test_prod_rejects_chain_id_mismatch(monkeypatch) -> None:
    ...
    tx = sign_tx_envelope_dict(
        tx={"tx_type": "POST_CREATE", "signer": "@alice", "nonce": 1, "payload": {"body": "hello"}, "chain_id": "chain-B"},
